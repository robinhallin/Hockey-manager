...
const MATCH_BRIEF_GOALS={
 continuity:{name:'Följ vår nuvarande plan',risk:'Kontinuitet behåller invanda uppgifter, men tidigare svagheter kan finnas kvar. Följ både skapade och insläppta lägen.'},
...
function matchBriefScout(club){
 const recent=(state.rivals?.clubs?.[club]?.recent||[]).filter(g=>g.year===state.season.year&&(!g.date||g.date<=state.calendar.date)&&!g.partial).slice(-5);
...
function matchBriefChoose(key){
 ...
 const p=MATCH_PLANS[def.plan];
 if(p){state.tactic=p.tactic;for(const field of ['attackStyle','forecheck','tempo'])state.tacticalPlan[field]=p[field];state.tacticalPlan.shotChoice=def.shotChoice;}
...
function matchBriefReport(m,training=true){
 ...
 const observation=b.goal==='continuity'?'Båda sidor av lägesbalansen visas. Det finns ingen förutbestämd förbättringsriktning för en oförändrad plan.':!comparable?'Vi samlar underlag för att bedöma uppgiften.':Math.abs(change)<0.5?'Ingen tydlig skillnad mot utgångsläget ännu.':(metric==='against'?change<0:change>0)?'Utfallet går i uppgiftens riktning.':'Utfallet går emot uppgiften. Granska assistentens observationer och formationerna innan du ändrar.';
 const risk=b.goal!=='continuity'&&comparable&&(metric==='against'?riskChange<-.5:riskChange>.5)?' Samtidigt syns planens risk i lägesbalansen åt andra hållet.':'';
...
function matchBriefTraining(key){
 if(!COACH_FOCUSES[key])return;
 if(coachFocus()){deskNavigate('home');officePanelTab('followup');return;}
