...
function setRecruitFilter(key,value){const f=recruitFilters();f[key]=['maxAge','maxFee','minAttribute'].includes(key)?Number(value):String(value);queueInterfaceSave();render();}
...
function createScoutMission(){
 ...
 const filters={...r.filters},candidates=recruitCandidates(filters).filter(p=>(state.scoutReports[String(p.id)]?.visits||0)<3&&!scoutPending(p.id)).slice(0,3);
...
function recruitTab(tab){if(!['needs','search','missions','shortlist','deals','history','free','world','loans'].includes(tab))return;deskNavigate('transfers',tab);}
