...
function recruitCounterDifferences(d){
 if(!d?.counter||!d.original)return [];const rows=[];for(const [key,label,format] of [['fee','Övergångssumma',money],['salary','Årslön',money],['years','Avtalslängd',v=>v+' år'],['role','Roll',v=>v]])if(d.original[key]!==d.counter[key])rows.push({key,label,from:format(d.original[key]),to:format(d.counter[key])});return rows;
}
