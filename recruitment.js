...
function recruitmentWeek(){
 ...
 const r=state.recruitment;r.weeks++;advanceRecruitment();
...
function recruitmentPlayerView(){
 ...
 return `<section class="recruitment-page"><button class="btn secondary" onclick="recruitTab('search')">← Rekrytering</button><header class="daily-heading"><div><span class="career-eyebrow">${trainingSafe(club)} · ${RECRUIT_COUNTRIES[recruitCountry(club)]}${p.fictional?' · FIKTIV SPELARE':''}</span><h1>${trainingSafe(p.name)}</h1><p>${p.pos} · ${p.age} år · ${p.contractYears} år kvar på avtalet</p></div><button class="btn secondary" onclick="toggleRecruitShortlist('${p.id}')">${r.shortlist.some(id=>samePlayerId(id,p.id))?'Ta bort från önskelistan':'Lägg till i önskelistan'}</button></header>${assessmentPanel(p)}
