...
function recruitCanSell(p,club){if(p.futureContract)return false;if(club===WORLD_FREE)return true;const roster=state.clubRosters[club]||[];const group=q=>q.pos==='MV'?'MV':q.pos==='B'?'B':'F';return roster.filter(q=>group(q)===group(p)).length>({MV:2,B:6,F:12})[group(p)];}
...
function recruitmentPlayerView(){
 ...
 return `<section class="recruitment-page"><button class="btn secondary" onclick="recruitTab('search')">← Rekrytering</button><header class="daily-heading"><div><span class="career-eyebrow">${trainingSafe(club)} · ${RECRUIT_COUNTRIES[worldIsFree(p.id)?p.nationality:recruitCountry(club)]||p.nationality||'Sverige'}${p.fictional?' · FIKTIV SPELARE':''}</span><h1>${trainingSafe(p.name)}</h1><p>${p.pos} · ${p.age} år · ${p.contractYears} år kvar på avtalet</p></div><button class="btn secondary" onclick="toggleRecruitShortlist('${p.id}')">${r.shortlist.some(id=>samePlayerId(id,p.id))?'Ta bort från önskelistan':'Lägg till i önskelistan'}</button></header>${assessmentPanel(p)}${medicalPlayerPanel(p)}${calendarFuturePanel(p)}
