...
function scoutingView(){
  ...
  return `<section class="bench-hub"><span class="panel-label">SCOUTING & TRÄNARTEAM</span><h1>Ditt beslutsunderlag</h1><p>Personalens kompetens och kunskap om spelaren påverkar rapporten. Observationer blir klara när du spelar nästa omgång.</p><div class="staff-grid">${state.staff.map(s=>`<article class="unit-card"><h2>${s.name}</h2><p>Specialisering: ${s.specialty}</p><div class="row"><span>Bedöma förmåga</span><b>${s.ability}/20</b></div><div class="row"><span>Bedöma potential</span><b>${s.potential}/20</b></div></article>`).join('')}</div><h2>Bevakade spelare</h2>${reports.length?reports.map(({p,r})=>`<button class="scout-report-row" onclick="state.selectedMarketPlayer='${p.id}';state.page='marketPlayer';render()"><strong>${p.name}</strong><span>${assessmentShort(p)}</span><span>${r.dueRound?'Observation pågår':`${r.visits} observationer klara`}</span></button>`).join(''):'<p>Öppna en spelare på transfermarknaden och beställ din första observation.</p>'}<button class="btn" onclick="state.page='transfers';render()">Till transfermarknaden</button></section>`;
}
