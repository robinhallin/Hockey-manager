...
function overviewDecisionsView(items){
 const visible=overviewUI.decisions?items:items.slice(0,3);
 ...
 }).join('')||'<div class="ov-empty"><strong>Du är i fas.</strong><p>Inga prioriterade ärenden just nu. Planera veckan eller fortsätt till nästa dag.</p></div>'}</div>${items.length>3?`<button type="button" class="ov-more" id="overview-more-decisions" aria-expanded="${Boolean(overviewUI.decisions)}" onclick="overviewExpandDecisions()">${overviewUI.decisions?'Visa färre ärenden':`Visa alla ${items.length} ärenden · ${items.length-3} till`}</button>`:''}</section>`;
}
...
let overviewReadCache={key:null,value:null};
function overviewReadModel(){
 const key=[state.calendar?.date,state.round,state.live?.minute,state.live?.second,state.training?.messages?.length,state.recruitment?.incoming?.length,state.history?.length,state.page].join('|');
 if(overviewReadCache.key===key&&overviewReadCache.value)return overviewReadCache.value;
 const items=overviewDecisionItems(),fixture=overviewFixture(),watch=overviewWatchPlayers(),roster=managerRoster(),table=leagueTable();
 return overviewReadCache={key,value:{items,fixture,watch,roster,table,ready:roster.filter(medicalReady).length}};
}
function overviewDashboardView(){
 const {items,fixture,watch,roster,ready}=overviewReadModel();
