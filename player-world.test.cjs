...
run("globalThis.memoryPlayer=managerRoster()[0];memoryPlayer.games=100;memoryPlayer.goals=30;memoryPlayer.assists=70;careerPlayerMilestones(memoryPlayer,managerClub());globalThis.memories=careerHistoryPlayer(memoryPlayer)");
assert.ok(run("memories.some(e=>e.type==='games'&&e.value===100)"));assert.ok(run("memories.some(e=>e.type==='points'&&e.value===100)"));
run("careerRecord('test-record','Testrekord',12,memoryPlayer.name,{playerId:memoryPlayer.id});careerRecord('test-record','Testrekord',10,'Annan')");assert.equal(run("ensureCareerHistory().records['test-record'].value"),12);
