...
function matchSetMatchup(key,value){
 if(!state.live||state.live.finished||!['matchupLine','matchupTarget','defensiveFaceoffLine','offensiveFaceoffLine','tiredOpponentLine'].includes(key)||!['0','1','2','3',...(key==='matchupLine'?['none']:[])].includes(value))return;
...
function matchLineView(){
 ...
 return `<div class="mc-matchup-controls">${select('matchupLine','Vår matchningskedja',true)}${select('matchupTarget','Mot deras kedja',false)}${select('defensiveFaceoffLine','Defensiv tekning',false)}${select('offensiveFaceoffLine','Offensiv tekning',false)}${select('tiredOpponentLine','Efter deras icing',false)}</div><p class="mc-note">${matching?'Matchningen prioriteras när kedjan är redo; andra får färre byten.':'Fri rotation.'} Hemmalaget väljer sist.</p>${matchLineTable(ledger,true)}<p class="mc-note" role="status">${trainingSafe(matchDesk.notice||e.teams[0].matchupReason||'Matchningen prövas vid nästa byte.')}</p>`;
}
