...
assert.equal(r('state.playerWorld.freeAgents.length'),6);
assert.equal(r('new Set([...Object.values(state.clubRosters).flat(),...state.playerWorld.freeAgents].map(p=>p.id)).size'),r('Object.values(state.clubRosters).flat().length+6'));
assert.ok(r('state.playerWorld.freeAgents.every(p=>p.contractYears===0&&p.research.stats.some(s=>Number.isFinite(HA_LEAGUE_LEVEL[s.league])&&s.season==="25-26"))'));
