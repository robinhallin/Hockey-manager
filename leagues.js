...
function leagueBackground(g){
 ...
 const result=rivalSimulate(g);
 const h=result.homeGoals,v=result.awayGoals,ot=result.overtime;
 Object.assign(g,{played:true,homeGoals:h,awayGoals:v,overtime:ot,shootout:result.shootout,duration:result.duration,rivalRows:result.rows,rivalReports:result.reports});
 a.gp++;b.gp++;a.gf+=h;a.ga+=v;b.gf+=v;b.ga+=h;
 const win=h>v?a:b,lose=h>v?b:a;if(ot){win.otw++;lose.otl++;win.pts+=2;lose.pts++;}else{win.w++;lose.l++;win.pts+=3;}
 leagueRecordBackground(g);
}
