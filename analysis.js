...
function analysisSnapshot(){
 ...
 return {id:a.id,year:state.season?.year||2026,round:state.round,club:managerClub(),opponent:m.opponent,home:state.schedule.find(g=>g.round===state.round&&(g.home===managerClub()||g.away===managerClub()))?.home===managerClub(),stage:state.season?.phase==='playoffs'?SEASON_STAGES[state.season.stage]:'Grundserie',own:m.hv,against:m.opp,finished:m.finished,hockey:m.rink?.hockey?{counts:m.rink.hockey.counts,stops:m.rink.hockey.stops}:null,partial:a.partial,shootout:Boolean(m.analysisShootout),abandoned:Boolean(m.analysisAbandoned),shots:a.shots,events:a.events,units:Object.values(a.units),players:Object.values(a.players).map(p=>{const current=playerById(p.id);return {...p,endAttributes:{...(current?.attributes||p.startAttributes)}};})};
}
...
function analysisSamples(){const s=state.analysis;return s.window==='season'?s.matches.filter(m=>m.year===(state.season?.year||2026)):s.window==='all'?s.matches:s.matches.slice(0,Number(s.window)||10);}
