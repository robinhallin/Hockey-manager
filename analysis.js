...
function finishAnalysis(){
 ...
 const snapshot=analysisSnapshot();a.saved=true;state.analysis.matches.unshift(JSON.parse(JSON.stringify(snapshot)));trimAnalysisArchive();state.analysis.selected='latest'; storiesAfterMatch(snapshot);coachMatchDone(snapshot);
...
function performanceGrade(row,partial=false){
 ...
 return {stars:Math.round(Math.max(1,Math.min(5,score))*2)/2,reason};
}
function finishPerformance(){
 ...
 const rows=Object.values(m.leagueBox?.players||{}).filter(p=>p.seconds>0).map(p=>({...p,...performanceGrade(p,partial)}));
...
function performanceView(report){
 ...
 const rows=[...report.rows].sort((a,b)=>a.club.localeCompare(b.club)||(b.stars??0)-(a.stars??0)||b.seconds-a.seconds);
 return `<section class="performance-report"><h2>Matchens spelarinsatser</h2><p>Betyg för just denna match, från en till fem stjärnor. Poäng, avslut, disciplin och spel i lika styrka vägs ihop; målvakter bedöms på räddningar. Kort istid och små målvaktsunderlag lämnas utan betyg. Förmåga och potential är separata bedömningar.</p><div class="mc-table-scroll"><table><caption>Prestationsbetyg · ${report.date?calText(report.date):'Matchrapport'}</caption><thead><tr><th>Spelare</th><th>Lag</th><th>Matchbetyg</th><th>Bakom betyget</th></tr></thead><tbody>${rows.map(r=>`<tr><th scope="row">${trainingSafe(r.name)} <small>${r.pos}</small></th><td>${trainingSafe(r.club)}</td><td>${performanceStars(r.stars)}</td><td>${trainingSafe(r.reason)}</td></tr>`).join('')||'<tr><td colspan="4">Ingen registrerad istid. Oanvända reserver får inga betyg.</td></tr>'}</tbody></table></div></section>`;
}
...
function coachEvidence(){return (state.analysis?.matches||[]).filter(coachEligible).slice(0,3);}
...
function coachAdopt(key){
 ...
 state.analysis.coachFocus={key,club:managerClub(),year:state.season.year,date:state.calendar.date,baseline:matches.map(m=>({id:m.id,value:coachMeasure(m,key)})),seen:(state.analysis.matches||[]).map(m=>m.id),sessions:[],results:[]};
 save();render();
}
function coachPlan(){
 const f=coachFocus();if(!f||f.results.length>=3)return;ensureTrainingData();
...
function coachTrainingDone(log){const f=coachFocus();if(!f||f.results.length>=3||!log.trained||log.type!==COACH_FOCUSES[f.key].session||f.sessions.some(s=>s.date===log.date))return;f.sessions.push({date:log.date,trained:log.trained,resting:log.resting});f.sessions=f.sessions.slice(-20);}
function coachMatchDone(m){
 const f=coachFocus();if(!f||!coachEligible(m)||f.results.length>=3||f.seen.includes(m.id)||f.results.some(r=>r.id===m.id))return;
 f.results.push({id:m.id,opponent:m.opponent,value:coachMeasure(m,f.key)});
 if(f.results.length===3)managerMessage('coach-review:'+m.id,'Dags att följa upp ditt träningsfokus','Tre matcher är spelade. Jämför utfallet med utgångsläget i tränarens uppföljning. Resultaten påverkas också av motstånd, istid och matchbild.','Assisterande tränare',{link:'training'});
}
