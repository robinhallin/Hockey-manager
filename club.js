...
function ensureClub(){
 ...
 if(!state.clubOffice){
  ...
  state.clubOffice={version:1,year:clubYear(),opening:state.money,ledger:[],totals:{},settled:[],priority:'balanced',ticket:220,capacity:clubArena()?.capacity||Math.round(fans*1.2),sponsor:Math.round(annualWageCost()*.6+(leagueOf()==='HA'?2500000:6000000)),operations:leagueOf()==='HA'?4000000:9000000,staffLimit:leagueOf()==='HA'?2600000:3200000,market:[],taken:[],archives:[],message:'',offer:null};
...
function clubTrainingFactor(){return state.clubOffice?.priority==='first'?1.1:clubProjectFactor('training');}
function clubJuniorFactor(){return state.clubOffice?.priority==='youth'?1.15:clubProjectFactor('junior');}
