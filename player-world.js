...
function worldStartingFree(){return [...WORLD_START_FREE,...(ALLSVENSKAN_DATABASE.outside||[]).filter(e=>e.status==='free').map(e=>({...e.player,previousClub:e.previousClub,departure:e.source}))].map(row=>{const p=haPlayer({...row,registration:'Kontraktslös'},row.previousClub);Object.assign(p,{club:WORLD_FREE,previousClub:row.previousClub,contractYears:0,freeSince:2026});p.research.checked=ALLSVENSKAN_DATABASE.checked;p.research.departure=row.departure;p.research.freeSnapshot=true;return p;});}
