...
// Senior completion no longer advances the junior league or junior loans; dated J20 fixtures own that progression.
run('state.live.hv=3;state.live.opp=1;finishMatch(false)');
assert.equal(run('p.academy.loan.remaining'),8);assert.equal(run('state.juniors.matches.length'),0);
assert.equal(run('p.games'),0);assert.equal(run('p.goals'),0);
run('juniorFixture("manual-j20:1")');assert.equal(run('p.academy.loan.remaining'),7);assert.equal(run('state.juniors.matches.length'),1);const fixture=run('JSON.stringify(state.juniors.matches)');run('juniorFixture("manual-j20:1")');assert.equal(run('JSON.stringify(state.juniors.matches)'),fixture);
