...
function feedbackNews(key,club,kind,title,body){
 ...
 const titleParts=Array.isArray(title)?title:undefined,bodyParts=Array.isArray(body)?body:undefined;
 f.news.unshift({id:f.nextId++,key,club,league:leagueOf(club),year:recruitmentYear(),date:state.calendar.date,kind,title:referencePlainText(title),body:referencePlainText(body),...(titleParts?{titleParts}:{}),...(bodyParts?{bodyParts}:{})});f.news=f.news.slice(0,120);
}
function feedbackRivalEvent(e){
 if(['coach','injury'].includes(e.kind)||e.kind==='development'&&/får chansen|flyttas upp|A-laget/.test(e.title))feedbackNews(e.id,e.club,e.kind,e.titleParts||e.title,e.textParts||e.text);
}
...
function feedbackAfterMatch(m){
 ...
 for(const row of m.players.filter(p=>(p.goals||0)+(p.assists||0)>=2)){
  const p=managerRoster().find(p=>samePlayerId(p.id,row.id));if(p?.age<=21)feedbackNews('youth:'+m.year+':'+m.id+':'+p.id,m.club,'development',playerHeadline(p,' visar framfötterna'),`${row.goals||0} mål och ${row.assists||0} assist mot ${m.opponent}. En enskild match är ett tidigt tecken, inte ett säkert genombrott.`);
 }
}
...
const staffReviewUI={tab:'planning'};
function staffReviewTab(tab){if(!['planning','arrivals','coaching'].includes(tab))return;staffReviewUI.tab=tab;render();queueInterfaceSave();}
function staffReviewView(){
 ...
 const promises=[...(state.training?.promises||[]).map(p=>({...p,minutes:15})),...managerRoster().filter(p=>p.recruitmentPromise).map(p=>({...p.recruitmentPromise,name:p.name,playerId:p.id}))];
 return `<section class="feedback-page staff-workspace"><header class="workspace-heading"><div><span class="desk-kicker">${trainingSafe(managerClub())} · TRÄNARTEAM</span><h1>Stab & uppföljning</h1></div></header><nav class="workspace-tabs" aria-label="Stabens arbetsområde">${[['planning','Truppbehov'],['arrivals','Förstärkningar & löften'],['coaching','Träning & taktik']].map(([tab,label])=>`<button aria-pressed="${staffReviewUI.tab===tab}" onclick="staffReviewTab('${tab}')">${label}</button>`).join('')}</nav><section ${staffReviewUI.tab==='planning'?'':'hidden'}><button class="btn" onclick="state.managerFeedback.selectedBrief=null;feedbackRefresh()">Uppdatera stabsunderlaget</button>${f.selectedBrief&&!brief?'<p>Det äldre underlaget finns inte längre i rapportarkivet. Uppdatera för en aktuell bedömning.</p>':feedbackBriefView(brief)}<details><summary>Tidigare stabsunderlag</summary>${f.briefs.filter(b=>b.club===managerClub()).map(b=>`<p><button class="btn secondary" onclick="state.managerFeedback.selectedBrief=${b.id};save();render()">${trainingSafe(b.date)} · ${trainingSafe(b.profile||'Kontinuitet')}</button></p>`).join('')}</details></section><section ${staffReviewUI.tab==='arrivals'?'':'hidden'}><h2>Genomförda förstärkningar</h2>${f.followups.filter(r=>r.club===managerClub()).map(r=>`<article class="feedback-item"><h3>${playerReference(r.playerId,r.name)} · ${trainingSafe(r.profile)}</h3><p>${trainingSafe(r.date)} · ${r.games}/5 matcher · ${Math.round(r.seconds/60)} minuter · ${r.points} poäng · ${r.excused} medicinskt undantagna matcher</p><p>Bedömd lucka vid ankomst: ${r.before}. Vid senaste match: ${r.after}.</p><p>${trainingSafe(r.result||'Uppföljning pågår. Träningsmatcher och ofullständiga matcher räknas inte.')}</p>${deskLink('Se över laguttagningen',{page:'lines'})}</article>`).join('')||'<p>Nya värvningar, lån och uppflyttningar följs här efter genomförd flytt.</p>'}<h2>Speltidslöften</h2>${promises.map(p=>`<article class="feedback-item"><strong>${playerReference(p.playerId,p.name)}</strong><p>${p.qualified}/${rolePromiseRule(p).required} matcher med minst ${p.minutes} minuter · ${p.games}/${rolePromiseRule(p).total} bedömda matcher · ${trainingSafe(p.resolved?(p.result||'Avslutat'):'Pågår')}</p></article>`).join('')||'<p>Inga registrerade speltidslöften.</p>'}${deskLink('Spelarnas förtroende',{page:'locker'})}</section><section ${staffReviewUI.tab==='coaching'?'':'hidden'}><h2>Tränings- och taktisk uppföljning</h2>${coachFocus()?feedbackCoachView(coachFocus()):'<p>Välj ett mätbart fokus i matchanalysen för att börja följa effekten av arbetet.</p>'}${deskLink('Välj fokus och granska matcher',{page:'statistics'})}<details><summary>Tidigare fokus (${f.coachHistory.length})</summary>${f.coachHistory.filter(c=>c.club===managerClub()).map(feedbackCoachView).join('')}</details><p>${deskLink('Följ säsongens överenskommelser och historier',{page:'stories'})}</p></section></section>`;
}
function feedbackNewsView(){
 ...
 return `<section class="card feedback-page"><h1>Liganyheter</h1><p>Rapporter från genomförda händelser i din spelvärld.</p><div class="feedback-filters">${[['all','Alla'],['SHL','SHL'],['HA','Hockeyallsvenskan'],['own','Min klubb']].map(([key,label])=>`<button class="btn secondary" aria-pressed="${f.filter===key}" onclick="state.managerFeedback.filter='${key}';save();render()">${label}</button>`).join('')}</div>${rows.map(n=>`<details class="league-news-row"><summary><small>${trainingSafe(n.date)} · ${trainingSafe(n.club)}</small><strong>${playerReferenceText(n.titleParts||n.title)}</strong></summary><p>${playerReferenceText(n.bodyParts||n.body)}</p></details>`).join('')||'<p>Nya händelser visas när spelvärlden går vidare.</p>'}<details><summary>Klubbens tidigare notiser</summary>${(state.news||[]).map(n=>`<p>${trainingSafe(n)}</p>`).join('')}</details></section>`;
}
