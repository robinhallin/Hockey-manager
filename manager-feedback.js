...
function feedbackNewsView(){
 ...
 return `<section class="card feedback-page"><h1>Liganyheter</h1><p>Rapporter från genomförda händelser i din spelvärld.</p><div class="feedback-filters">${[['all','Alla'],['SHL','SHL'],['HA','Hockeyallsvenskan'],['own','Min klubb']].map(([key,label])=>`<button class="btn secondary" aria-pressed="${f.filter===key}" onclick="state.managerFeedback.filter='${key}';save();render()">${label}</button>`).join('')}</div>${rows.map(n=>`<article class="feedback-item"><small>${trainingSafe(n.date)} · ${trainingSafe(n.club)}</small><h2>${trainingSafe(n.title)}</h2><p>${trainingSafe(n.body)}</p></article>`).join('')||'<p>Nya händelser visas när spelvärlden går vidare.</p>'}<details><summary>Klubbens tidigare notiser</summary>${(state.news||[]).map(n=>`<p>${trainingSafe(n)}</p>`).join('')}</details></section>`;
}
