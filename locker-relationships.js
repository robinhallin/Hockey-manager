...
function relationshipAfterMatch(){
 ...
 for(const p of managerRoster()){
  ...
  if(c.eligible>=6&&c.tension>=70&&!c.wantsMove){c.wantsMove=true;relationshipEvent(p.name+' överväger sin framtid',c.evidence+' Spelaren begär högre lön vid förlängning så länge konflikten kvarstår.');managerMessage('relationship-move:'+c.id,p.name+' överväger att lämna',c.evidence+' Försök återställa ansvar eller diskutera en mindre roll.','Omklädningsrum',{link:'locker'});}
...
function relationshipAction(id,action){
 ...
 if(action==='listen'){
  ...
  if(c.plan)return;c.status='following';c.plan={startEligible:c.eligible};socialRemember(p,'En tydlig väg tillbaka',`Tränaren prioriterar rollen: avtalat kedjeansvar eller motsvarande istid i tre raka bedömbara matcher. Fyra bedömbara matcher utan den förändringen ger −2 i förtroende. Medicinska undantag gäller.`);
 }else{
