...
function managerJ20TrainingSessions(row){return row?.version===2?row.sessions||0:0;}
function managerJ20TrainingRecord(player,before,session,key){
  ...
  const old=store[id];
  const row=old?.version===2?old:{version:2,legacySessions:old?.sessions||0,playerId:player.id,playerName:player.name,started:state.calendar?.date||null,sessions:0,hardSessions:0,recoverySessions:0,restSessions:0,startFatigue:before.fatigue,startProfile:before.profile,lastDate:null,lastKey:null,types:{}};
  row.playerName=player.name;
  if(before.participated){row.sessions++;row.hardSessions+=session?.intensity==='hard'?1:0;row.types[session?.type||'unknown']=(row.types[session?.type||'unknown']||0)+1;}
  else {row.restSessions++;row.recoverySessions+=session?.type==='recovery'?1:0;}
...
function managerJ20TrainingRecommendation(player,row){
  const readiness=managerJ20Readiness(player),gain=row?.profileGain||0,sessions=managerJ20TrainingSessions(row),fatigue=player?.fatigue||0;
  if(row&&row.version!==2)return {level:'Nytt träningsunderlag behövs',detail:'Äldre passräkning skilde inte träning från vila. Historiken behålls, men nya deltagna pass krävs för träningsmilstolpen.',action:'continue',readiness};
