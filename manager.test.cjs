...
assert.equal(run('clubAIState("HV71").academy.roster.map(p=>p.id).join()'),run('academyIds'));
assert.equal(run('state.recruitment.ai["HV71"].cash'),run('oldCash'));
assert.ok(run('state.juniors.roster.every(p=>!clubAIState("HV71").academy.roster.some(q=>q.id===p.id))'));
assert.equal(run('state.managerCareer.status'),'employed');
