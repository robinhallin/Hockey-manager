...
assert.equal(r('dayTransition.phase'),'ready');
...
assert.ok(r("dayTransition.rows.some(row=>row.kind==='message'&&row.title.includes('rapport'))"));
const after=r('JSON.stringify(state)');
r('continueGame();dayTransitionRender()');
...
assert.equal(m('dayTransition.phase'),'ready');
