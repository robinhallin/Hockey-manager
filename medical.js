...
function medicalPlayerPanel(p){
 if(!isOwnPlayer(p)&&!(state.juniors?.roster||[]).some(q=>samePlayerId(q.id,p.id)))return '<p>Medicinska uppgifter är inte tillgängliga för externa spelare.</p>';
...
function medicalRenderDecision(){
 ...
 const dialog=document.getElementById('medical-decision-dialog');
 try{if(dialog?.showModal)dialog.showModal();else dialog?.setAttribute('open','');}
