...
function ensureMedical(){
 ...
 for(const roster of Object.values(state.clubRosters))for(const p of roster)if(!p.health)p.health={load:0,injury:null,clearance:'rest'};
}
...
function medicalCallUp(pos){
 ...
 const id=state.medical.nextId++,p={id:`medical-academy-${state.season?.year||2026}-${id}`,name:`${['Albin','Hugo','Viktor','Leo'][id%4]} ${['Lindgren','Bergström','Sund','Lind'][Math.floor(id/4)%4]}`,pos,age:18,fictional:true,nationality:'SWE',overall:66,potential:79,shooting:65,passing:66,defense:65,physical:66,salary:350000,value:700000,contractYears:3,squadRole:'Breddspelare',promisedRole:'Breddspelare',happiness:70,morale:70,fatigue:0,goals:0,assists:0,games:0,shots:0,pim:0};
 managerRoster().push(p);ensurePlayerAttributes(p);ensureManagementData();ensureMedical();ensureLocker();repairMedicalLines();medicalNotice(`${p.name} flyttas upp. Treårsavtal, 350 000 kr per år. Lönebudgeten påverkas.`);
}
