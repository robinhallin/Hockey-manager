...
function careerVisibleClubs(){return Object.keys(CLUB_DATA).filter(name=>(careerFilter==='all'||careerIdentity(name).group===careerFilter)&&(careerLeague==='all'||(ALLSVENSKAN_CLUBS.some(c=>c[0]===name)?'HA':'SHL')===careerLeague));}
function setCareerLeague(value){if(!['all','SHL','HA'].includes(value))return;careerLeague=value;if(!careerVisibleClubs().includes(careerChoice))careerChoice=careerVisibleClubs()[0];render();}
let careerMessage = '';
