...
let careerLeague = 'all';
function careerVisibleClubs(){return Object.keys(CLUB_DATA).filter(name=>(careerFilter==='all'||careerIdentity(name).group===careerFilter)&&(careerLeague==='all'||(swissClub(name)?'CH_NL':ALLSVENSKAN_CLUBS.some(c=>c[0]===name)?'HA':'SHL')===careerLeague));}
function setCareerLeague(value){if(!['all',...Object.keys(LEAGUE_NAMES)].includes(value))return;careerLeague=value;if(!careerVisibleClubs().includes(careerChoice))careerChoice=careerVisibleClubs()[0];render();}
let careerMessage = '';
const CAREER_SAVE_KEY = 'hockey_manager_alpha02';
const PREVIOUS_CAREER_KEY = 'hockey_manager_previous_career';

const CLUB_PALETTES={
 'HV71':['#093e78','#f2cf51'],'Brynäs IF':['#3b1720','#f3c882'],'Djurgårdens IF':['#17375d','#efbf65'],'Färjestad BK':['#15503f','#e0bf65'],'Frölunda HC':['#6d202e','#eeafaa'],'Linköping HC':['#163b70','#a8c9f6'],'Luleå Hockey':['#3c2025','#efc84f'],'Malmö Redhawks':['#69242d','#f3aea7'],'Rögle BK':['#155541','#b7dfb4'],'Skellefteå AIK':['#292821','#f0d143'],'Timrå IK':['#682630','#aacaef'],'Växjö Lakers':['#1e446b','#f5ab65'],'Örebro Hockey':['#6c242d','#f0b8b1'],'Björklöven':['#1a5940','#e3d16b'],
 'AIK':['#292923','#f0d668'],'Almtuna IS':['#6a2936','#efacb8'],'BIK Karlskoga':['#164c79','#c3e0f9'],'IK Oskarshamn':['#164a76','#f0ca5f'],'Kalmar HC':['#692b2e','#f0b8a1'],'Leksands IF':['#1a4778','#d7e9ff'],'MoDo Hockey':['#6c2730','#eeaaa6'],'Mora IK':['#67272d','#b4d99c'],'Nybro Vikings':['#602a2e','#efb789'],'Södertälje SK':['#184875','#edcc70'],'Vimmerby HC':['#302e21','#e9d27b'],'Visby/Roma':['#283c35','#d3e1cd'],'Västerås IK':['#302e22','#efd570'],'Östersunds IK':['#1c513c','#c9e4c3']
};
function careerIdentity(name){const c=CAREER_CLUBS[name]||CAREER_CLUBS.HV71,p=CLUB_PALETTES[name]||['#233e50',c.color];return {...c,color:p[1],primary:p[0]};}
function applyClubTheme(){
 const club=careerScreen==='select'||careerScreen==='review'?careerChoice:managerClub(),c=careerIdentity(club);
 const shell=document.querySelector('.game-shell');shell?.setAttribute?.('style',`--club-primary:${c.primary};--club-accent:${c.color};--desk-accent:${c.color};--club-color:${c.color}`);shell?.setAttribute?.('data-club',club);
}
...
function careerOffer(name,rosters){
  const c=leagueCareerOffer(careerIdentity(name),name,rosters),roster=rosters[name]||(swissClub(name)?swissRoster(name):[]);
...
function beginCareerSelection(){
  ...
  careerDraft=newState();careerChoice=managerClub();careerFilter='all';careerLeague='all';careerScreen='select';careerMessage='';render();
}
...
function acceptCareer(){
  ...
  if(state.careerStarted){
    try{careerStore(PREVIOUS_CAREER_KEY,JSON.stringify(state));}
...
function previousCareer(){
  ...
  try{
    const previous=careerRead(careerStorage.getItem(PREVIOUS_CAREER_KEY));
...
function previousCareerName(){try{const p=careerRead(careerStorage.getItem(PREVIOUS_CAREER_KEY));return p?.careerStarted&&CLUB_DATA[p.managerClub]?p.managerClub:null;}catch{return null;}}
...
function careerMenuView(){
  ...
  return `<section class="career-menu">${careerHeader()}<div class="career-menu-body"><div class="career-menu-copy"><span class="career-eyebrow">SVENSK HOCKEY. DITT NÄSTA KAPITEL.</span><h1>DITT LAG.<br>DINA <em>BESLUT.</em></h1><p>Från första laguttagningen till den sista slutsignalen. Bygg laget du tror på.</p><div class="career-menu-actions">${state.careerStarted?`<button class="career-resume" onclick="resumeCareer()"><span><small>FORTSÄTT KARRIÄR</small><strong>${managerClub()}</strong><span>${seasonLabel()} · Omgång ${state.round} · ${state.live&&!state.live.finished?'Match pausad':'Klubbkontoret'}</span></span><b aria-hidden="true">↗</b></button>`:''}<p class="career-database-note">NY STARTDATABAS · ${PLAYER_EVIDENCE_DATA.coverage.rosterPlayers} verkliga spelare i SHL och Allsvenskan + ${PLAYER_EVIDENCE_DATA.coverage.freeAgents} scoutbara kontraktslösa · truppunderlag från 23 september 2026, simulerad karriärstart 1 augusti · attributmodell granskad 23 september. Officiellt kompletterande statistikunderlag för ${PLAYER_EVIDENCE_DATA.coverage.officialPlayers} spelare. Fem träningsmatcher före ett nytt simulerat serieschema med premiär 10 september; verkliga resultat importeras inte. Gäller nya karriärer; din sparade karriär behålls.</p><button class="career-text-button" onclick="showSaveFiles()">Sparfiler, spelguide & felrapport</button><button class="career-primary" onclick="beginCareerSelection()">Starta ny karriär <span aria-hidden="true">→</span></button>${previous?`<button class="career-text-button previous-career" onclick="previousCareer()">Öppna föregående karriär · ${previous}</button>`:''}</div>${careerMessage?`<p role="alert">${careerMessage}</p>`:''}</div><div class="arena-caption"><span>01 / FÖRE NEDSLÄPP</span><p>Allt börjar<br>med ett beslut.</p></div></div><footer class="career-menu-footer"><span>42 KLUBBAR <i>•</i> 52 OMGÅNGAR <i>•</i> DIN IDENTITET</span><span>HOCKEY MANAGER / BETA ${HOCKEY_BETA_VERSION}</span></footer></section>`;
}
