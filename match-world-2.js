...
const MatchWorld2=(()=>{
 ...
 function backgroundShotContext({creation,resistance,shooterPosition,pp,plan={},opposition={}},rand){
  ...
  return {d,angle,pressure,screen,oneTimer:false,rebound:false,behind:false,lateralSpeed:0,worldModel:2};
 }
...
// functions. These wrappers intentionally preserve the current numeric balance.
...
if(typeof CareerBroadcastMatch!=="undefined"){
 ...
 CareerBroadcastMatch.prototype.actionOptions=function(a){
  ...
  // A deliberately small shared-model tie breaker. Spatial geometry remains the
  // main signal, while the same control/pressure edge used by background games
  // can separate otherwise near-equal decisions.
...
if(typeof rivalSimulate==='function'){
 ...
 rivalSimulate=function(game){
  const result=baseRivalSimulate(game);result.modelVersion=2;
  for(const report of result.reports||[])report.modelVersion=2;
  return result;
 };
}
