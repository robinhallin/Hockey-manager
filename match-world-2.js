...
const MatchWorld2=(()=>{
 ...
 function livePlan(team){
  const plan=team.plan||{};
  return {style:team.attackStyle||plan.style||'control',posture:team.posture||plan.posture||'balanced',forecheck:team.forecheck||plan.forecheck||'balanced',tempo:team.tempo||plan.tempo||'normal',shotChoice:team.tactics?.shotChoice||plan.shotChoice||'balanced',pp:team.tactics?.pp||plan.pp||'131',pk:team.tactics?.pk||plan.pk||'box'};
 }
 function tacticalIntent(plan={}){
  const choice=plan.shotChoice||'balanced',posture=plan.posture||'balanced';
 ...
 function profileFromAttributes(attrs,plan={},count=5,goalie=null){
  ...
  return {version:2,count,attrs:a,control:controlScore(a),attack:attackScore(a),defense:defenseScore(a),transition:transitionScore(a),discipline:a.discipline,
   ...
   plan:{style:plan.style||plan.mentality||'balanced',forecheck:plan.forecheck||'balanced',tempo:plan.tempo||'normal',posture:plan.posture||'balanced',shotChoice:plan.shotChoice||'balanced',pp:plan.pp||'131',pk:plan.pk||'box'}};
 }
 ...
 function decisionValues(attrs,plan={},context={}){
  ...
  const attackMentality=plan.posture==='attack'?1:plan.posture==='defense'?-.8:style==='attacking'||style==='pressure'?1:style==='counter'?.35:style==='defensive'?-.8:0;
 ...
 function liveProfiles(match){
  ...
  const signature=`${JSON.stringify((match.teams||[]).map(livePlan))}:${match.tick}:${match.actors?.map(a=>`${a.id}:${a.role}:${Math.round((a.player?.energy??100)*10)}`).join('|')}`;
  ...
  const result=[0,1].map(side=>{
   ...
   const team=match.teams[side]||{},plan=livePlan(team);
 ...
 function liveDecisionValues(match,a){
  ...
  const team=match.teams[a.side]||{},plan=livePlan(team);
