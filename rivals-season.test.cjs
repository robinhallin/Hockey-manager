...
run('for(state.round=1;state.round<=52;state.round++){calendarToMatch();for(const g of state.schedule.filter(g=>g.round===state.round))leagueBackground(g);calendarAfterFixture();}');
...
run('enterPlayoffs();globalThis.steps=0;while(state.season.phase==="playoffs"&&steps++<100){if(currentSeasonFixture()){createMatch();state.live.hv=4;state.live.opp=1;finishMatch(false);}else watchRemainingPlayoffs();}');
