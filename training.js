...
function managerMessagePriority(m){
 if(m.decisionType&&!m.resolved)return 'decision';
 if(m.priority==='high'||['Styrelsen','Medicinskt team'].includes(m.category)&&m.required)return 'recommendation';
 if(['Veckoplanering','Motståndsanalys','Chefsscout','Junioransvarig','Spelarutveckling'].includes(m.category))return 'recommendation';
 return 'information';
}
function managerMessage(key,title,body,category='Tränarteam',extra={}){
  ...
  if(Array.isArray(title)){extra={...extra,titleParts:title};title=referencePlainText(title);}
  if(Array.isArray(body)){extra={...extra,bodyParts:body};body=body.map(part=>typeof part==='string'?part:part.text||'').join('');}
...
function trainingTarget(p,session='skills'){
  const a=ensurePlayerAttributes(p),focus=TRAINING_FOCUSES[p.developmentFocus],rolePlan=developmentRolePlan(p);
  if(rolePlan&&p.developmentFocus==='Balanserad'){const keys=rolePlan.keys.filter(k=>Object.hasOwn(a,k));if(keys.length)return keys.slice().sort((x,y)=>(p.trainingProgress?.[x]||0)-(p.trainingProgress?.[y]||0)||a[x]-a[y])[0];}
...
function createPlayerConversation(){
  ...
  if(!candidate)return;
  managerMessage(`talk:${state.round}`,playerHeadline(candidate,' vill prata om sin roll'),'Jag vill få mer ansvar. Kan du ge mig minst 15 minuters istid i två av de tre kommande matcherna?','Spelarsamtal',{playerId:candidate.id,decisionType:'minutes'});
}
...
function trainingPlayerPanel(p,compact=false){
  ...
  const background=`${developmentPanel(p)}${p.lastPerformance?`<div class="player-last-performance"><strong>Senaste matchinsats</strong> ${performanceRating(p.lastPerformance)}<p>${calText(p.lastPerformance.date)} · ${trainingSafe(p.lastPerformance.opponent)} · ${trainingSafe(p.lastPerformance.reason)}</p></div>`:''}`;
  return `${compact?'':background}<div class="individual-training"><div class="training-individual-fields">${p.pos!=='MV'?`<label>Utvecklingsmål<select onchange="setDevelopmentRolePlan('${p.id}',this.value)"><option value="">Inget rollmål</option>${Object.entries(DEVELOPMENT_ROLE_PLANS).filter(([k])=>PLAYER_TASKS[k]?.groups.includes(p.pos==='B'?'defense':'forwards')).map(([k,d])=>`<option value="${k}" ${p.developmentRolePlan===k?'selected':''}>${d.name}</option>`).join('')}</select></label>`:''}<label>Individuellt fokus<select onchange="setDevelopmentFocus('${p.id}',this.value)">${focusOptions(p).map(f=>`<option ${p.developmentFocus===f?'selected':''}>${f}</option>`).join('')}</select></label><label>Träningsbelastning<select onchange="setIndividualLoad('${p.id}',this.value)">${[['normal','Följ lagets pass'],['light','Lätt träning'],['rest','Individuell vila']].map(([v,l])=>`<option value="${v}" ${p.trainingLoad===v?'selected':''}>${l}</option>`).join('')}</select></label></div><div class="training-progress-label"><span>Nästa fokus: ${fields[key]}</span><strong>${Math.floor(p.trainingProgress[key]||0)} %</strong></div><progress max="100" value="${p.trainingProgress[key]||0}" aria-label="Utvecklingsarbete inom ${fields[key]}"></progress><p>Framsteg mot nästa attributsteg. Utvecklingstakten beror på ålder, ork och tränarstöd. Individuell vila gäller tills du ändrar den eller din planerade återgång infaller.</p>${changes.length?`<p class="training-growth">Utvecklat sedan uppföljningen började: ${changes.map(k=>`${fields[k]} +${p.attributes[k]-p.trainingBaseline[k]}`).join(', ')}.</p>`:''}</div>${trainingPlanningPanel(p)}${compact?`<details class="dv-report"><summary>Utveckling & senaste matchinsats</summary>${background}</details>`:''}`;
}
...
function inboxView(){
  ...
  const messages=t.messages.filter(m=>inboxUI.filter==='all'||inboxUI.filter==='unread'&&(!m.read||inboxUI.detail&&m.id===t.selectedMessage)||inboxUI.filter==='decisions'&&managerMessagePriority(m)==='decision'||inboxUI.filter==='recommendations'&&managerMessagePriority(m)==='recommendation'||inboxUI.filter==='reports'&&managerMessagePriority(m)==='information');
  return `<section class="manager-inbox inbox-workspace ${inboxUI.detail?'reading':''}"><header class="daily-heading"><div><span class="career-eyebrow">KLUBBKONTORET</span><h1>Din inkorg.</h1><p>${unread} olästa meddelanden · rapporter, samtal och nästa beslut.</p></div><button class="btn secondary" onclick="markManagerInboxRead()">Markera rapporter som lästa</button></header><nav class="inbox-filters" aria-label="Filtrera inkorgen">${[["all","Alla",t.messages.length],["unread","Olästa",unread],["decisions","Beslut krävs",t.messages.filter(m=>managerMessagePriority(m)==='decision').length],["recommendations","Rekommendation",t.messages.filter(m=>managerMessagePriority(m)==='recommendation').length],["reports","Information",t.messages.filter(m=>managerMessagePriority(m)==='information').length]].map(([key,label,count])=>`<button aria-pressed="${inboxUI.filter===key}" onclick="inboxFilter('${key}')">${label} <span>${count}</span></button>`).join('')}</nav><div class="inbox-layout"><nav id="inbox-message-list" class="inbox-list" aria-label="Meddelanden">${messages.map(m=>`<button ${m.id===selected?.id?'id="inbox-selected-row"':''} class="inbox-item ${m.id===selected?.id?'selected':''} ${m.read?'':'unread'}" onclick="openManagerMessage(${m.id})"><span>${m.category}<small>${m.date?calText(m.date):`OMG ${m.round}`}</small></span><strong>${trainingSafe(m.title)}</strong><p>${managerMessagePriority(m)==='decision'?'BESLUT KRÄVS':managerMessagePriority(m)==='recommendation'?'REKOMMENDATION':'INFORMATION'} · ${m.read?'Läst':'Oläst'}</p></button>`).join('')||'<p class="inbox-empty">Inga meddelanden i detta urval.</p>'}</nav><article id="inbox-letter" tabindex="-1" class="inbox-letter"><button class="inbox-back" onclick="inboxBack()">← Till meddelandelistan</button>${selected&&(inboxUI.detail||messages.some(m=>m.id===selected.id))?`<header><span class="career-eyebrow">${trainingSafe(selected.category)} · ${selected.date?calText(selected.date):`Omgång ${selected.round}`}</span><h2>${selected.titleParts?playerReferenceText(selected.titleParts):selected.playerId!==undefined?playerReference(selected.playerId,selected.title):trainingSafe(selected.title)}</h2></header><div class="message-body">${playerReferenceText(selected.bodyParts||selected.body).split('\n').map(line=>`<p>${line}</p>`).join('')}</div>${selected.decisionType==='minutes'&&!selected.resolved?`<section class="manager-decision"><h3>Vad svarar du?</h3><button class="btn" onclick="answerPlayerConversation(${selected.id},'promise')">Jag lovar dig mer istid</button><p>Minst 15 minuter i två av de tre kommande matcherna.</p><button class="btn secondary" onclick="answerPlayerConversation(${selected.id},'honest')">Jag kan inte lova en större roll</button></section>`:''}${selected.outcome?`<div class="manager-outcome">${trainingSafe(selected.outcome)}</div>`:''}<footer>${messageActionView(selected)}</footer>`:'<div class="inbox-empty"><h2>Välj ett meddelande</h2><p>Rapporter och samtal samlas här. Välj ett urval och öppna det du vill läsa.</p></div>'}</article></div></section>`;
}
...
function messageOpenContext(id){
 ...
 if(m.incomingId&&state.recruitment.incoming.some(o=>o.id===m.incomingId)){officeOpenDeal('incoming:'+m.incomingId);return;}
 ...
 if(m.playerId!==undefined&&playerIdentity(m.playerId)){deskOpenPlayer(m.playerId);if(m.link==='scouting'){profileWorkspace.tab='overview';recruitHub.panel='report';render();deskBrowserBefore();}if(['medical','training'].includes(m.link)){profileWorkspace.tab='development';render();deskBrowserBefore();}return;}
