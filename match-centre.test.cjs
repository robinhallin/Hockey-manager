...
const friendly=boot();friendly.run('startCareerWithClub("HV71");calendarInitialPreseason();calendarBookFriendly("AIK",calAdd(state.calendar.date,3));calendarContinue();calendarContinue();calendarContinue();calendarContinue();(!state.live&&(state.calendar.date=calendarTarget()),startMatch());trackIceTime(60)');assert.equal(friendly.run('state.live.leagueBox.friendly'),true);assert.ok(friendly.run('Object.values(state.live.leagueBox.players).some(p=>p.club==="AIK"&&p.seconds>0)'));friendly.run('calendarFinishFriendly()');assert.equal(friendly.run('Object.keys(state.leagueStatistics.rows).length'),0);
