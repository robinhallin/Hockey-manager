...
run('(state.calendar.date=calendarTarget(),createMatch());deskNavigate("match")');
...
run('(!state.live&&(state.calendar.date=calendarTarget()),startMatch());liveStep();globalThis.clock=analysisClock();globalThis.score=state.live.hv+":"+state.live.opp;matchTab("tactics")');
...
run('startCareerWithClub("HV71");(state.calendar.date=calendarTarget(),createMatch());state.live.analysis.shots=[{side:"own",outcome:"goal"},{side:"own",outcome:"post"},{side:"own",outcome:"wide"},{side:"own",outcome:"rebound"},{side:"opponent",outcome:"save"}];globalThis.snapshot=JSON.stringify([state.live.rink,state.lines,state.calendar.date]);render();render()');
...
for(const club of run('Object.keys(state.world.membership)')){
 run(`startCareerWithClub(${JSON.stringify(club)});(state.calendar.date=calendarTarget(),createMatch());deskNavigate('match');matchTab('changes')`);assert.equal(run('managerClub()'),club);assert.doesNotMatch(get('#content').innerHTML,/undefined|NaN/);
...
run('startCareerWithClub("Rögle BK");(state.calendar.date=calendarTarget(),createMatch());globalThis.steps=0;while(!state.live.finished&&steps++<1600){if(!state.live.running){while(medicalPending())medicalDecisionAccept();startMatch();}if(steps%70===0){matchTab("tactics");matchPlan(steps%140?"pressure":"protect");matchTarget("ice");matchFeedback("support");(!state.live&&(state.calendar.date=calendarTarget()),startMatch());}liveStep();}');
...
const friendly=boot();friendly.run('startCareerWithClub("HV71");useHistoricalCalendarFixture();calendarInitialPreseason();calendarBookFriendly("AIK",calAdd(state.calendar.date,3));calendarContinue();calendarContinue();calendarContinue();calendarContinue();(!state.live&&(state.calendar.date=calendarTarget()),startMatch());trackIceTime(60)');assert.equal(friendly.run('state.live.leagueBox.friendly'),true);assert.ok(friendly.run('Object.values(state.live.leagueBox.players).some(p=>p.club==="AIK"&&p.seconds>0)'));friendly.run('calendarFinishFriendly()');assert.equal(friendly.run('Object.keys(state.leagueStatistics.rows).length'),0);
