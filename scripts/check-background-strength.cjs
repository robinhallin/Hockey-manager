...
r("startCareerWithClub('HV71');globalThis.initial=JSON.stringify(state);globalThis.originalLineup=rivalLineup;globalThis.originalChance=MatchWorld2.backgroundAttemptChance;aiCoachDecision=(club,base)=>({...base});");
const results={};
for(const mode of ['ungated','selective','safe']){
 r(`state=JSON.parse(initial);rivalLineup=(...args)=>{const l=originalLineup(...args);l.plan.counter=${JSON.stringify(mode==='safe'?'safe':'selective')};return l;};MatchWorld2.backgroundAttemptChance=${mode==='ungated'?'args=>originalChance({...args,pk:false})':'originalChance'};`);
