...
const root=path.resolve(__dirname,'..'),out=process.env.LOCKER_SCREENSHOTS||'/tmp/hockey-locker';
...
(async()=>{
 ...
 try{
  ...
  await page.goto('http://127.0.0.1:'+server.address().port);await page.evaluate(()=>{startCareerWithClub('HV71');deskNavigate('locker');document.activeElement?.blur()});
  for(const [width,height] of [[1440,900],[1366,768],[1280,720],[1920,1080],[390,844]]){
   await page.setViewportSize({width,height});
   for(const [key,name] of [['situation','Läget i gruppen'],['leadership','Roller & ledarskap'],['promises','Löften & samtal']]){
    await page.getByRole('navigation',{name:'Omklädningsrummets vyer'}).getByRole('button',{name,exact:true}).click();
    await page.screenshot({path:path.join(out,`locker-${key}-${width}.png`),fullPage:true});
    assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+2),true,key+' width '+width);
    if(width>=1280){const panel=await page.locator('.ld-inspector').boundingBox();assert.ok(panel.y+panel.height<=height-15,key+' inspector fits '+width+': '+JSON.stringify(panel));}
   }
  }
  await page.setViewportSize({width:1366,height:768});await require('./locker-browser-checks.cjs').checkLocker(page);
  assert.deepEqual(errors,[]);console.log('PASS: all three views at 390–1920 px, desktop inspector, selection, sorting, filters, conversation, guards, promises, leadership and profile context. '+out);
