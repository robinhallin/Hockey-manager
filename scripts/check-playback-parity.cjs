...
for(const mode of ['full','extended','highlights','commentary']){
 r(`state=JSON.parse(initial);state.live.rink.mode=${JSON.stringify(mode)};state.live.speed=4;globalThis.e=studioEngine();globalThis.steps=0;while(e.time<3600&&!state.live.finished&&steps++<100000){studioPlaybackRate(e,state.live);state.live.running=true;studioStep();}`);
