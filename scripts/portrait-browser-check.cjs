...
(async()=>{
 ...
 try{
  ...
  assert.ok(loaded.length>=101);assert.ok(loaded.every(p=>p.width===384));
  ...
  assert.deepEqual(errors,[]);console.log(`PASS: ${loaded.length} portraits load, HV71, Brynäs, Frölunda and Färjestad profiles render, missing file falls back, no page errors`);
