...
(async()=>{
 ...
 try{
  ...
  assert.ok(loaded.length>=126);assert.ok(loaded.every(p=>p.width===384));
  ...
  await page.evaluate(()=>deskOpenPlayer('ep-10018'));
  await page.waitForFunction(()=>document.querySelector('.fm-profile-header .player-avatar img')?.naturalWidth>0);
  assert.match(await page.locator('.fm-profile-header .player-avatar').getAttribute('aria-label'),/Henrik Tömmernes/);
  await page.evaluate(()=>deskOpenPlayer('ep-10538'));
  await page.waitForFunction(()=>document.querySelector('.fm-profile-header .player-avatar img')?.naturalWidth>0);
  assert.match(await page.locator('.fm-profile-header .player-avatar').getAttribute('aria-label'),/Magnus Nygren/);
  await page.evaluate(()=>deskOpenPlayer('ep-11070'));
  await page.waitForFunction(()=>document.querySelector('.fm-profile-header .player-avatar img')?.naturalWidth>0);
  assert.match(await page.locator('.fm-profile-header .player-avatar').getAttribute('aria-label'),/Philip Holm/);
  assert.deepEqual(errors,[]);console.log(`PASS: ${loaded.length} portraits load, HV71, Brynäs, Frölunda, Färjestad and Djurgården profiles render, missing file falls back, no page errors`);
