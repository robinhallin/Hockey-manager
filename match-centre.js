...
function matchScoringView(){
 const m=state.live,goals=(m.analysis?.events||[]).filter(e=>e.type==='goal');
 const row=e=>`<li class="${e.side}"><time>${e.period?`P${e.period} · ${e.clock}`:analysisTime(e.time)}</time><b>${e.own!==undefined?`${e.own}–${e.against}`:''}</b><span><strong>${trainingSafe(e.scorer||e.text)}</strong><small>${e.scorer?`${e.side==='own'?trainingSafe(managerClub()):trainingSafe(m.opponent)} · ${e.assists?.length?'Assist: '+e.assists.map(p=>trainingSafe(p.name)).join(', '):'Utan assist'}`:'Äldre mål: assistuppgift saknas'}</small></span></li>`;
 return `<section class="mc-scorers" aria-label="Målskyttar och assist">${goals.length?`<ol>${goals.slice(-2).map(row).join('')}</ol>${goals.length>2?`<details><summary>Alla ${goals.length} mål</summary><ol>${goals.slice(0,-2).map(row).join('')}</ol></details>`:''}`:'<p>Målskyttar och assist visas här vid mål.</p>'}${m.analysisShootout?'<p>Avgjort på straffar. Det avgörande lagmålet räknas inte som ett spelarmål.</p>':''}</section>`;
}
...
function matchCentreView(){
 ...
 return `<div class="mc-page"><header class="mc-header"><div class="mc-score"><div class="mc-club"><span class="mc-badge" style="--club-color:${own.color}">${own.code}</span><strong>${trainingSafe(managerClub())}</strong></div><div class="mc-result"><span>${m.finished?'SLUTRESULTAT':interval?'PERIODPAUS':m.running?'LIVE':'PAUSAT'}</span><b>${m.hv}<i>–</i>${m.opp}</b><small>${period} · ${gameTime()}</small></div><div class="mc-club"><span class="mc-badge" style="--club-color:${opp.color}">${opp.code}</span><strong>${trainingSafe(m.opponent)}</strong></div></div><div class="mc-playback ${m.finished?'':'mc-live-controls'}">${m.finished?'<button class="btn" onclick="state.analysis.selected=\'latest\';coachingNavigate(\'statistics\')">Matchrapport →</button><button class="btn secondary" onclick="state.live=null;deskNavigate(\'home\')">Till kontoret</button>':`<button class="btn mc-play" id="match-play" onclick="matchPlay()">${m.running?'Ⅱ Pausa':interval?'▶ Starta perioden':studioActive()&&!studioEngine().started?'▶ Starta matchen':'▶ Fortsätt'}</button><label>Visning<select onchange="rinkMode(this.value)" aria-label="Matchvisning"><option value="full" ${m.rink.mode==='full'?'selected':''}>${studioActive()?'Alla sekvenser':'Hela matchen'}</option><option value="highlights" ${m.rink.mode==='highlights'?'selected':''}>${studioActive()?'Matchsändning':'Höjdpunkter'}</option></select></label><label>Hastighet<select onchange="setSpeed(this.value)" aria-label="Uppspelningshastighet">${[[1,'1×'],[2,'2×'],[3,'3×']].map(([v,t])=>`<option value="${v}" ${m.speed===v?'selected':''}>${t}</option>`).join('')}</select></label>`}</div></header>${matchScoringView()}<div class="mc-situation"><strong>${skaters('own')} mot ${skaters('opponent')}${hockeySpecial('own')==='pp'?' · Powerplay':hockeySpecial('own')==='pk'?' · Boxplay':''}</strong><span>${penalties.length?penalties.map(([club,t])=>`${trainingSafe(club)} ${analysisTime(typeof t==='number'?t:t.seconds??t.remaining)}`).join(' · '):'Inga utvisningar'}${m.goaliePulled?' · Eget mål tomt':''}${m.aiGoaliePulled?' · Motståndarens mål tomt':''}</span></div>
