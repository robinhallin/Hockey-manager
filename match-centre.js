...
function matchLivePanel(){return matchDesk.tab==='changes'?matchChangesTabs()+matchLineView():matchDesk.tab==='stats'?matchDetailedStats():matchDesk.tab==='players'?matchPlayersView():`<div class="mc-events">${state.live.events.map(e=>`<p>${trainingSafe(e.text)}</p>`).join('')}</div>`;}
