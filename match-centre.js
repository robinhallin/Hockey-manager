...
function matchCentreView(){
 ...
 <div class="mc-layout"><div class="mc-ice-column">${rinkView(true)}<div class="mc-on-ice"><span>PÅ ISEN · ${specialUnitOnIce()?'SPECIAL TEAMS':`KEDJA ${m.currentLine+1} · BACKPAR ${m.currentDefensePair+1}`}</span><p>${[...currentLinePlayers(),...currentDefensePlayers()].map(p=>`<span class="${matchEnergy(p)<55?'tired':''}">${trainingSafe(p.name.split(' ').at(-1))} <small>${Math.round(matchEnergy(p))}% energi</small></span>`).join('')}</p></div>
