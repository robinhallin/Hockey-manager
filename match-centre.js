...
function matchTab(tab){if(!['feedback','tactics','changes','lineup','stats','players','events','analysis','report','settings'].includes(tab))return;matchDesk.tab=tab;matchDesk.notice='';if(!['stats','players','events','analysis'].includes(tab)||matchPreferences().stats)matchPause();render();matchFocus('match-tab-'+tab);}
...
function matchStatCard(label,values,suffix=''){return `<div class="mc-stat"><span>${label}</span><strong><b>${values[0]}${Number.isFinite(values[0])?suffix:''}</b><i>–</i><b>${values[1]}${Number.isFinite(values[1])?suffix:''}</b></strong></div>`;}
...
function matchCoachView(){return matchDeskCoachView();}
...
function matchCentreView(){
 ...
 if(matchDesk.match!==m){matchDesk.match=m;matchDesk.notice='';matchDesk.tab='feedback';matchDesk.target='team';matchDesk.coachMode='coach';matchDesk.changesMode='units';matchDesk.scroll={};}
 ensureRink();return matchDeskView();
}
