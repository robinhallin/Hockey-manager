...
function matchTab(tab){if(!['feedback','tactics','changes','lineup','stats','players','events','settings'].includes(tab))return;matchDesk.tab=tab;matchDesk.notice='';if(!['stats','players','events'].includes(tab)||matchPreferences().stats)matchPause();render();matchFocus('match-tab-'+tab);}
...
function matchPlan(key,coachDecision=null){
...
function matchOrder(key,value,coachDecision=null){
 ...
 tacticalReviewRecord(before,'Lagorder',coachDecision);
...
function matchAction(action){
 ...
 if(action==='timeout'){if(m.timeoutUsed)return;useTimeout();matchNotice('Timeout tagen. Båda lagen har återhämtat sig. Ge laget ett budskap eller justera matchplanen.');}
...
function matchChangesView(){
 if(matchDesk.changesMode==='matchup')return matchChangesTabs()+matchLineView();
 ...
 const key=(StudioHockey.penaltyCount(m.penaltiesOpp)>StudioHockey.penaltyCount(m.penaltiesHV)?'pp':'pk')+((m.rotationIndex||0)%2+1);
 return `${matchChangesTabs()}${blocked?'<p class="mc-warning" role="status">Icing: laget får inte byta före nedsläpp.</p>':''}<h3>${special?'Special teams':'Skicka in nästa formation'}</h3><p class="mc-note">Energi återhämtas under faktisk bänktid. Belastningen följer med efter matchen. Rotation tar över efter nästa byte.</p><div class="mc-bench-actions"><button id="match-action-timeout" class="btn secondary" onclick="matchAction('timeout')" ${m.timeoutUsed?'disabled':''}>${m.timeoutUsed?'Timeout använd':'Ta timeout'}</button><button id="match-action-goalie" class="btn secondary" onclick="matchAction('goalie')" ${blocked?'disabled':''}>${m.goaliePulled?'Sätt in målvakten':'Ta ut målvakten'}</button></div>${timeoutDecisionView()}${special?`<div class="mc-units">${[0,1].map(i=>unit('special',i,state.specialTeams[key.slice(0,2)+(i+1)],i===(m.rotationIndex||0)%2)).join('')}</div>`:`<div class="mc-units">${[0,1,2,3].map(i=>unit('forwards',i,state.lines.forwards.slice(i*3,i*3+3),i===m.currentLine)).join('')}</div><div class="mc-units mc-pairs">${[0,1,2].map(i=>unit('defense',i,state.lines.defense.slice(i*2,i*2+2),i===m.currentDefensePair)).join('')}</div>`}
 <details class="mc-edit-players" ${matchDesk.editPlayers?'open':''} ontoggle="matchDesk.editPlayers=this.open"><summary>Ändra spelare i vald formation</summary><div class="mc-player-picks">${special?state.specialTeams[key].map((id,i)=>select(key,i,id,skaters,`${key.toUpperCase()} · plats ${i+1}`)).join(''):state.lines.forwards.slice(m.currentLine*3,m.currentLine*3+3).map((id,i)=>select('forwards',m.currentLine*3+i,id,skaters,['Vänsterforward','Center','Högerforward'][i])).join('')+state.lines.defense.slice(m.currentDefensePair*2,m.currentDefensePair*2+2).map((id,i)=>select('defense',m.currentDefensePair*2+i,id,skaters,`Back ${i+1}`)).join('')}
 ${select('goalie',0,state.lines.goalie,goalies().filter(medicalAvailable),'Målvakt')}</div></details><p class="mc-note">${m.goaliePulled?'Tomt mål: extra utespelare på isen.':'Timeout ger återhämtning. Uttagen målvakt ger en extra utespelare men lämnar målet tomt.'}</p><button class="mc-text-button" onclick="matchTab('lineup')">Öppna hela laguttagningen →</button>`;
}
function matchCoachView(){
 const m=state.live;
 return `<aside class="mc-coach" aria-label="Coachbänken"><header><span class="career-eyebrow">COACHBÄNKEN</span><h2>${m.finished?'Samla laget':'Ditt nästa beslut'}</h2><p>${m.finished?'Matchen är slut. Summera insatsen med spelarna.':m.running?'Statistik, istid och logg kan följas under spel. Tränarbeslut pausar matchen.':medicalPending()?medicalPending().name+' kan inte fortsätta. Hantera spelarbeskedet innan matchen startas.':trainingSafe(m.pauseReason||'Matchen är pausad.')+' Gör dina ändringar och tryck Fortsätt.'}</p></header>${m.finished?'':`<nav class="mc-tabs" aria-label="Coachval">${[['feedback','Matchcoach'],['tactics','Taktik'],['changes','Byten'],['lineup','Kedjor'],['stats','Statistik'],['players','Istid'],['events','Logg'],['settings','Inställningar']].map(([key,label])=>`<button id="match-tab-${key}" onclick="matchTab('${key}')" aria-pressed="${matchDesk.tab===key}">${label}</button>`).join('')}</nav>`}<div class="mc-coach-content">${matchDesk.notice&&!(matchDesk.tab==='changes'&&matchDesk.changesMode==='matchup')?`<p class="mc-response" role="status">${trainingSafe(matchDesk.notice)}</p>`:''}${m.running&&!matchReadOnlyTab()?`<div class="mc-follow">${matchEvidenceView()}<span>FÖLJ MATCHEN</span><h3>${HOCKEY_STYLES[hockeyStyle('own')]}</h3><p>${({attack:'Offensiv',balanced:'Balanserad',defense:'Defensiv'})[state.tactic]} · ${({low:'Lågt',normal:'Normalt',high:'Högt'})[state.tacticalPlan.tempo]} speltempo</p><p>Spelarna följer dina order. Öppna Matchcoach, Taktik eller Byten när du vill ingripa.</p>${m.benchFeedback?.history?.[0]?`<p>Senaste rop: ${trainingSafe(MATCH_MESSAGES[m.benchFeedback.history[0].kind]?.[0]||'Budskap')}</p>`:''}</div>`:m.finished||matchDesk.tab==='feedback'?matchFeedbackView():matchDesk.tab==='settings'?matchPreferencesView():matchDesk.tab==='tactics'?matchOrdersView():matchDesk.tab==='stats'?matchDetailedStats():matchDesk.tab==='players'?matchPlayersView():matchDesk.tab==='events'?`<div class="mc-events">${m.events.map(e=>`<p>${matchEventReference(e)}</p>`).join('')}</div>`:matchDesk.tab==='lineup'?lineupBoardView():matchChangesView()}${!m.finished&&matchDesk.tab==='stats'&&isPlayoffMatch()?seasonMatchPanel():''}${!m.finished&&['changes','lineup'].includes(matchDesk.tab)?tacticalReviewView(tacticalReviewSnapshot()):''}</div></aside>`;
}
...
function matchCentreView(){
 const m=state.live,venue=matchVenue();if(!m)return `<section class="mc-prematch"><span class="career-eyebrow">NÄSTA MATCH</span><h1>${trainingSafe(venue.home)} <span>–</span> ${trainingSafe(venue.away)}</h1><p>Förbered matchplanen. Före nedsläpp kan du prata med laget och välja formationer.</p><button class="btn" onclick="matchDesk.notice='';createMatch()">Till matchen →</button><h2>Matchplan</h2><p class="workspace-note">Välj en startplan eller justera lagorder. Ändringar gäller lagets matchplan och sparas direkt.</p>${matchOrdersView()}</section>`;
 if(matchDesk.match!==m){matchDesk.match=m;matchDesk.notice='';matchDesk.tab='feedback';matchDesk.target='team';matchDesk.coachMode='coach';matchDesk.changesMode='units';}
 ensureRink();const s=matchStats(),own=careerIdentity(venue.home),opp=careerIdentity(venue.away),skaters=side=>m.rink.actors.filter(a=>a.side===side&&a.pos!=='MV').length;
 ...
 return `<div class="mc-page ${m.finished?'mc-finished':'mc-live'}"><nav class="mc-utility"><button onclick="deskNavigate('home')">← Kontoret</button><button onclick="matchFullscreen()">Helskärm</button></nav><header class="mc-header"><div class="mc-score"><div class="mc-club"><span class="mc-badge" style="--club-color:${own.color}">${clubCrest(venue.home)}</span><strong>${trainingSafe(venue.home)}</strong><small>HEMMA</small></div><div class="mc-result"><span>${m.finished?'SLUTRESULTAT':interval?'PERIODPAUS':m.running?'LIVE':'PAUSAT'}</span><b>${matchVenueScore()[0]}<i>–</i>${matchVenueScore()[1]}</b><small>${period} · ${gameTime()}</small><small>${trainingSafe(venue.arena)}</small></div><div class="mc-club"><span class="mc-badge" style="--club-color:${opp.color}">${clubCrest(venue.away)}</span><strong>${trainingSafe(venue.away)}</strong><small>BORTA</small></div></div><div class="mc-playback ${m.finished?'':'mc-live-controls'}">${m.finished?'<button class="btn" onclick="state.analysis.selected=\'latest\';coachingNavigate(\'statistics\')">Matchrapport →</button><button class="btn secondary" onclick="state.live=null;deskNavigate(\'home\')">Till kontoret</button>':`<button class="btn mc-play" id="match-play" onclick="matchPlay()">${m.running?'Ⅱ Pausa':medicalPending()?'Hantera spelarbesked':interval?'▶ Starta perioden':studioActive()&&!studioEngine().started?'▶ Starta matchen':'▶ Fortsätt'}</button><label>Visning<select onchange="rinkMode(this.value)" aria-label="Matchvisning">${Object.entries(MATCH_VIEW_MODES).map(([v,label])=>`<option value="${v}" ${m.rink.mode===v?'selected':''}>${label}</option>`).join('')}</select></label>${matchPlaybackControls(m)}`}</div></header>${matchScoringView()}${!m.finished&&!m.running?`<p class="mc-pause-reason" role="status">${trainingSafe(medicalPending()?'Spelarbesked måste hanteras.':m.pauseReason||'Matchen väntar på att startas eller återupptas.')}</p>`:''}<div class="mc-situation"><strong>${skaters('own')} mot ${skaters('opponent')}${hockeySpecial('own')==='pp'?' · Powerplay':hockeySpecial('own')==='pk'?' · Boxplay':''}</strong><span>${matchPenaltyText()}${m.goaliePulled?' · Eget mål tomt':''}${m.aiGoaliePulled?' · Motståndarens mål tomt':''}</span></div>
 <section class="mc-stats" aria-label="Matchstatistik, hemmalaget först">${matchStatCard(m.analysis?.partial?'Registrerade skott på mål':'Skott på mål',matchVenueValues(s.shots))}${matchStatCard('Farliga chanser',matchVenueValues(s.danger))}${matchStatCard('Puckinnehav',matchVenueValues(s.possession),'%')}${matchStatCard('Vunna tekningar',matchVenueValues(s.faceoffs))}${matchStatCard('Powerplay · mål/försök',matchVenueValues(s.pp))}${matchStatCard('Räddningar',matchVenueValues(s.saves))}</section>
 ...
 ${m.finished?performanceView(m.performance)+`<details class="mc-details" ${matchDesk.details?'open':''} ontoggle="matchMore('details',this.open)"><summary>Matchstatistik · jämför lagen</summary>${matchDetailedStats()}</details><details class="mc-details" data-desk-fold="iceTime" ${deskFolds.iceTime?'open':''} ontoggle="deskFolds.iceTime=this.open"><summary>Spelarstatistik & istid · båda lagen</summary>${matchPlayersView()}</details><details class="mc-details" ${matchDesk.events?'open':''} ontoggle="matchMore('events',this.open)"><summary>Händelser & matchlogg (${m.events.length})</summary><div class="mc-events">${m.events.map(e=>`<p><time>P${e.period} · ${e.time}</time><span>${matchEventReference(e)}</span></p>`).join('')}</div></details>${isPlayoffMatch()?seasonMatchPanel():''}`:''}</div>${matchCoachView()}</div></div>`;
}
...
function matchReadOnlyTab(){return ['stats','players','events'].includes(matchDesk.tab)||(matchDesk.tab==='changes'&&matchDesk.changesMode==='matchup');}
function matchLivePanel(){return matchDesk.tab==='changes'?matchChangesTabs()+matchLineView():matchDesk.tab==='stats'?matchDetailedStats():matchDesk.tab==='players'?matchPlayersView():`<div class="mc-events">${state.live.events.map(e=>`<p>${matchEventReference(e)}</p>`).join('')}</div>`;}
function matchPenaltyText(){
 const m=state.live,rows=[...m.penaltiesHV.map(p=>[managerClub(),p]),...m.penaltiesOpp.map(p=>[m.opponent,p])];
 return rows.map(([club,p])=>`${trainingSafe(club)} · ${p.playerId!=null?playerReference(p.playerId,p.player||p.name):trainingSafe(p.player||p.name||'Utvisning')}${p.kind?' · '+trainingSafe(p.label||p.kind):''}${p.minutes?' · '+p.minutes+' min':''} · ${analysisTime(typeof p==='number'?p:p.seconds??p.remaining)} kvar${p.queued?' (väntar på att börja avtjänas)':''}${p.affectsStrength===false?' · personlig, lagets numerär oförändrad':''}`).join(' · ')||'Inga utvisningar';
}
