...
function matchCoachView(){
 const m=state.live;
 return `<aside class="mc-coach" aria-label="Coachbänken"><header><span class="career-eyebrow">COACHBÄNKEN</span><h2>${m.finished?'Samla laget':'Ditt nästa beslut'}</h2><p>${m.finished?'Matchen är slut. Summera insatsen med spelarna.':m.running?'Statistik, istid och logg kan följas under spel. Tränarbeslut pausar matchen.':medicalPending()?medicalPending().name+' kan inte fortsätta. Hantera spelarbeskedet innan matchen startas.':trainingSafe(m.pauseReason||'Matchen är pausad.')+' Gör dina ändringar och tryck Fortsätt.'}</p></header>${m.finished?'':`<nav class="mc-tabs" aria-label="Coachval">${[['feedback','Feedback'],['tactics','Taktik'],['changes','Byten'],['lineup','Kedjor'],['stats','Statistik'],['players','Istid'],['events','Logg'],['settings','Inställningar']].map(([key,label])=>`<button id="match-tab-${key}" onclick="matchTab('${key}')" aria-pressed="${matchDesk.tab===key}">${label}</button>`).join('')}</nav>`}<div class="mc-coach-content">${matchDesk.notice?`<p class="mc-response" role="status">${trainingSafe(matchDesk.notice)}</p>`:''}${m.running&&!matchReadOnlyTab()?`<div class="mc-follow">${matchEvidenceView()}<span>FÖLJ MATCHEN</span><h3>${HOCKEY_STYLES[hockeyStyle('own')]}</h3><p>${({attack:'Offensiv',balanced:'Balanserad',defense:'Defensiv'})[state.tactic]} · ${({low:'Lågt',normal:'Normalt',high:'Högt'})[state.tacticalPlan.tempo]} speltempo</p><p>Spelarna följer dina order. Öppna Feedback, Taktik eller Byten när du vill ingripa.</p>${m.benchFeedback?.history?.[0]?`<p>Senaste rop: ${trainingSafe(MATCH_MESSAGES[m.benchFeedback.history[0].kind]?.[0]||'Budskap')}</p>`:''}</div>`:m.finished||matchDesk.tab==='feedback'?matchFeedbackView():matchDesk.tab==='settings'?matchPreferencesView():matchDesk.tab==='tactics'?matchOrdersView():matchDesk.tab==='stats'?matchDetailedStats():matchDesk.tab==='players'?matchPlayersView():matchDesk.tab==='events'?`<div class="mc-events">${m.events.map(e=>`<p>${trainingSafe(e.text)}</p>`).join('')}</div>`:matchDesk.tab==='lineup'?lineupBoardView():matchChangesView()}${!m.finished&&matchDesk.tab==='stats'&&isPlayoffMatch()?seasonMatchPanel():''}${!m.finished&&['changes','lineup'].includes(matchDesk.tab)?tacticalReviewView(tacticalReviewSnapshot()):''}</div></aside>`;
}
