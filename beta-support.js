'use strict';
const HOCKEY_BETA_VERSION='0.1.0-beta.7';
const betaErrors=[];
...
function betaReportText() {
 return JSON.stringify({game:'Hockey Manager',version:HOCKEY_BETA_VERSION,platform:desktopBridge()?'desktop':'browser',reportedAt:new Date().toISOString(),club:managerClub(),date:state.calendar?.date,page:state.page,round:state.round,season:state.season?.year,match:state.live?{finished:!!state.live.finished,running:!!state.live.running}:null,saveFailed:careerSaveError,loadFailed:!!careerLoadIssue,performance:betaPerformanceReport(),errors:betaErrors,steps:'Beskriv vad du gjorde, vad du förväntade dig och vad som hände.'},null,2);
}
...
function betaSupportView() {
 ...
 <details><summary>Rapportera ett problem</summary><p>Skriv stegen som utlöste problemet, vad du förväntade dig och vad som hände. Bifoga gärna en skärmbild och den här felrapporten till projektägaren. Vid problem i en viss karriär kan du även exportera sparfilen separat. Inget skickas automatiskt.</p><button class="btn secondary" onclick="downloadBetaReport()">Exportera felrapport</button><p>Rapporten innehåller betaversion, klubb, speldatum, aktuell vy, registrerade fel och sammanfattade prestandamätningar. Den innehåller inte spelarvärden eller din fullständiga sparfil.</p></details>
 <details><summary>Betans omfattning och begränsningar</summary><p>Spelbara karriärer finns i de 28 svenska seniorklubbarna och i en separat schweizisk National League med 14 klubbar. ${swissRulesText()} Tjeckien och Finland är förberedda datapaket och ännu inte spelbara ligor. Spelschemat är simulerat och attribut, ekonomi och utvecklingsprognoser är spelmodeller. Juniorer och den nordamerikanska spelvärlden är inte en fullständigt verifierad verklig truppdatabas. Balans och spelglädje behöver återkoppling från provspelning.</p><p>Windows-paketet är avsett för 64-bitars Windows 10/11. Den första privata betan saknar kodsignering och automatisk uppdatering. Installera kommande version över den gamla; sparmappen ligger separat. Exportera gärna en egen kopia före uppdatering.</p></details></section>`;
}
