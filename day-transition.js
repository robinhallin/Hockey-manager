...
function dayTransitionSnapshot(){
 return {messages:new Set((state.training?.messages||[]).map(m=>m.id)),events:new Set((state.rivals?.events||[]).map(e=>e.id)),stories:new Set((state.stories?.active||[]).map(s=>s.id)),
...
function dayTransitionSummary(before){
 ...
 for(const s of state.stories?.active||[])if(!before.stories?.has(s.id))rows.push({kind:'story',id:s.id,date:s.created,label:'Säsongshistoria',title:s.title,detail:s.chapters?.at(-1)?.text||'',required:s.status==='decision'});
