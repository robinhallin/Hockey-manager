...
const RINK_LABELS={faceoff:'Tekning',buildup:'Uppspel',carry:'Pucktransport',pass:'Passning',turnover:'Puckvinst',shot:'Avslut',goal:'MÅL',save:'Räddning',post:'Ramträff',rebound:'Retur',block:'Blockerat skott',penalty:'Utvisning'};
...
function rinkSync(){
 ...
 try{
  ...
  r.actors=actors;
  if(!actors.some(a=>a.key===r.carrier)&&r.phase!=='rebound'){
...
function rinkDelay(){const m=state.live;return m?.rink?.mode==='highlights'&&!m.rink.hot?100:m?.speed===3?300:m?.speed===2?700:1400;}
...
function rinkEndFrame(){
 const r=state.live.rink;r.duration=Math.max(80,rinkDelay()*.88);if(!r.oppFatigue)r.oppFatigue={};
...
function rinkFaceoff(){
 ...
 r.owner=side;r.carrier=winner.key;r.zoneTicks=0;r.lastPass=null;r.restart=false;
 for(const teamSide of ['own','opponent']){let i=0;for(const a of r.actors.filter(a=>a.side===teamSide)){
  ...
  const d=teamSide==='own'?1:-1;a.x=rinkClamp(r.faceoffX-d*(a.pos==='B'?19:5));a.y=[25,50,75,33,67,50][i++%6];
 }}
 winner.x=rinkClamp(r.faceoffX+(ownWin?2:-2));winner.y=50;r.puck={x:winner.x,y:winner.y};
...
function rinkMoveTeam(){
 const r=state.live.rink,carrier=r.actors.find(a=>a.key===r.carrier);if(!carrier)return;
 const dir=r.owner==='own'?1:-1,tempo=r.owner==='own'?(state.tacticalPlan?.tempo==='high'?1.3:state.tacticalPlan?.tempo==='low'?.75:1):1;
 const advance=5+rinkAttribute(carrier,'skating')*.3;
 rinkMove(carrier,carrier.x+dir*advance*tempo,rinkClamp(carrier.y+(50-carrier.y)*.2,18,82));
 let ownIndex=0,otherIndex=0;
 const pressure=rinkPress(rinkOther(r.owner)),defensive=rinkOther(r.owner)==='own'&&state.tactic==='defense';
 for(const a of r.actors){
  if(a.key===carrier.key)continue;
  if(a.pos==='MV'){rinkMove(a,rinkX(a.side,8),50+(carrier.y-50)*.12);continue;}
  if(a.side===r.owner){
   const idx=ownIndex++,offense=a.side==='own'&&state.tactic==='attack'?5:0;
   const x=carrier.x+dir*(a.pos==='B'?-19:10+offense),y=[22,78,38,62,50][idx%5];rinkMove(a,rinkClamp(x,12,88),y);
  }else{
   const idx=otherIndex++,deep=a.pos==='B'||defensive;
   const x=deep?carrier.x+dir*(defensive?16:13):carrier.x-dir*(2+pressure*13),y=deep?50+(carrier.y-50)*.3+(idx%2?12:-12):carrier.y+(idx%2?7:-7);
   rinkMove(a,rinkClamp(x,12,88),rinkClamp(y,18,82));
  }
 }
 r.puck={x:carrier.x,y:carrier.y};
}
...
function rinkLose(defender,text){const r=state.live.rink;r.owner=defender.side;r.carrier=defender.key;r.lastPass=null;r.zoneTicks=0;r.puck={x:defender.x,y:defender.y};rinkSay(text,'turnover',rinkX(defender.side,defender.x)>65);}
...
function rinkTakeShot(actor){
 ...
 if(rinkRoll()<threat*.23){const blocker=[...defenders].sort((a,b)=>rinkDistance(a,actor)-rinkDistance(b,actor))[0];
  if(blocker){if(side==='own')m.blocksOpp++;else m.blocksHV++;r.puck={x:blocker.x,y:blocker.y};r.owner=blocker.side;r.carrier=blocker.key;r.lastPass=null;r.zoneTicks=0;rinkSay(`${blocker.name} blockerar ${actor.name}s skott.`,'block',true);return;}
 }
 ...
 if(outcome==='goal'){r.restart=true;r.faceoffX=50;}
 else if(outcome==='save'){r.restart=true;r.faceoffX=rinkX(side,76);}
 else {r.phase='rebound';r.puck={x:rinkX(side,84),y:rinkClamp(50+(rinkRoll()-.5)*26,18,82)};r.carrier=null;r.caption=outcome==='post'?`${actor.name} träffar ramen – pucken är lös!`:`Retur efter ${actor.name}s avslut!`;}
}
function rinkStep(){
 ensureRink();const m=state.live,r=m?.rink;if(!r||m.finished||!m.running)return;
 if(r.period!==m.period||r.ot!==(m.overtimePeriods||1)){r.period=m.period;r.ot=m.overtimePeriods||1;r.restart=true;r.faceoffX=50;}
 rinkBeginFrame();
 if(r.restart){rinkFaceoff();rinkEndFrame();return;}
 if(r.phase==='rebound'){
  const contenders=r.actors.filter(a=>a.pos!=='MV').map(a=>({a,score:rinkDistance(a,r.puck)-rinkAttribute(a,'positioning')*.18-rinkRoll()*3})).sort((a,b)=>a.score-b.score);
  const winner=contenders[0]?.a;if(winner){rinkMove(winner,r.puck.x,r.puck.y);rinkLose(winner,`${winner.name} tar hand om returen.`);}rinkEndFrame();return;
 }
 ...
 if(progress>64&&rinkRoll()<(r.zoneTicks>3?.8:.25+(progress-65)/100)){rinkTakeShot(actor);rinkEndFrame();return;}
 const options=rinkSkaters(r.owner).filter(a=>a.key!==actor.key).map(a=>({a,score:rinkPassChance(actor,a,defenders)+(rinkX(a.side,a.x)-progress)/150+rinkRoll()*.12})).sort((a,b)=>b.score-a.score);
 if(options.length&&rinkRoll()<.68){const target=options[0].a;r.puckVia={x:actor.x,y:actor.y};
  if(rinkRoll()<rinkPassChance(actor,target,defenders)){r.carrier=target.key;r.puck={x:target.x,y:target.y};r.passes[r.owner]++;r.lastPass={from:actor.key,id:actor.id,to:target.key,frame:r.frame};rinkSay(`${actor.name} hittar ${target.name} med en passning.`,'pass',progress>68);}
