...
function rinkAttribute(actor,key){
 ...
 const confidence=['decisions','composure','vision','positioning'].includes(key)?playerMoraleBonus(p)*.35:0;
 return attrClamp((ensurePlayerAttributes(p)[key]||10)+confidence-fatigue/25-(100-matchEnergy(p))/35+teamwork,1,20);
}
 ...
 try{
  ...
  for(const [side,players] of [['own',rinkOwnPlayers()],['opponent',rinkOpponentPlayers()]]){
   ...
   for(const p of players){const key=rinkKey(side,p),prior=old.find(a=>a.key===key),goalie=p.pos==='MV';
    ...
    if(inherited)borrowed.add(inherited.key);actors.push(prior?{...prior,...row,x:prior.x,y:prior.y}:old.length&&!r.restart?{...row,x:50,y:10}:row);
   }
  }
  r.actors=actors;
  if(!actors.some(a=>a.key===r.carrier)&&r.phase!=='rebound'&&!r.hockey?.loose){
   ...
   r.carrier=r.restart?candidates[0]?.key||null:null;r.lastPass=null;
   if(!r.restart)r.hockey.loose={type:'change',side:r.owner};
  }
...
function rinkEndFrame(){const r=state.live.rink;r.hockey.transition=Math.max(0,r.hockey.transition-1);r.duration=Math.max(80,rinkDelay());}
function rinkFaceoff(){
 ...
 r.hockey.pp=null;r.owner=side;r.carrier=winner.key;r.zoneTicks=0;r.lastPass=null;r.restart=false;r.hockey.loose=null;r.hockey.icingHold=null;
...
function rinkMove(actor,x,y){
 const distance=rinkDistance(actor,{x,y}),max=3.5+rinkAttribute(actor,actor.pos==='MV'?'movement':'skating')*.24,ratio=distance?Math.min(1,max/distance):1;
 const dx=(x-actor.x)*ratio,dy=(y-actor.y)*ratio;
 // Momentum softens direction changes; players brake as they reach their hockey assignment.
 const blend=distance<3?.85:.65;
 actor.vx=(actor.vx||0)*(1-blend)+dx*blend;actor.vy=(actor.vy||0)*(1-blend)+dy*blend;
 if(distance<.6){actor.vx=0;actor.vy=0;actor.x=x;actor.y=y;}else{actor.x+=actor.vx;actor.y+=actor.vy;}
 actor.x=rinkClamp(actor.x);actor.y=rinkClamp(actor.y,10,90);
}
...
function rinkLose(defender,text){const r=state.live.rink;r.hockey.transition=r.owner!==defender.side?3:r.hockey.transition;r.hockey.pp=null;r.owner=defender.side;r.carrier=defender.key;r.lastPass=null;r.zoneTicks=0;r.puck={x:defender.x,y:defender.y};rinkSay(text,'turnover',rinkX(defender.side,defender.x)>65);}
...
function rinkStep(){
 ...
 const pkDecision=hockeyPKDecision(actor);
 if(pkDecision==='clear'){hockeyClear(actor);rinkEndFrame();return;}
 if(pkDecision==='counter'&&r.hockey.counterOutlet){const target=r.actors.find(a=>a.key===r.hockey.counterOutlet);if(target&&rinkRoll()<rinkPassChance(actor,target,defenders)){r.puckVia={x:actor.x,y:actor.y};r.carrier=target.key;r.puck={x:target.x,y:target.y};r.passes[r.owner]++;r.lastPass={from:actor.key,id:actor.id,to:target.key,frame:r.frame};rinkSay(`${actor.name} ser en fri väg och hittar ${target.name} i en boxplaykontring.`,'pass',true);rinkEndFrame();return;}}
 if(hockeySpecial(r.owner)!=='pk'&&progress<45&&nearest&&rinkDistance(nearest,actor)<10&&rinkRoll()<(hockeyStyle(r.owner)==='counter'?.18:.06)){hockeyClear(actor);rinkEndFrame();return;}
 ...
 if(options.length&&rinkRoll()<(hockeySpecial(r.owner)==='pp'?.85:hockeyStyle(r.owner)==='control'?.78:.62)){const target=options[0].a;r.puckVia={x:actor.x,y:actor.y};
  if(rinkRoll()<rinkPassChance(actor,target,defenders)){if(hockeyEntry(r.owner,actor,target,actor.key)){rinkEndFrame();return;}r.carrier=target.key;r.puck={x:target.x,y:target.y};r.passes[r.owner]++;if(r.hockey.pp)r.hockey.pp.passes++;r.lastPass={from:actor.key,id:actor.id,to:target.key,frame:r.frame};rinkSay(`${actor.name} hittar ${target.name} med en passning.`,'pass',progress>68);}
  else {const blocker=[...defenders].sort((a,b)=>rinkLaneThreat(actor,target,[b])-rinkLaneThreat(actor,target,[a])||rinkDistance(a,target)-rinkDistance(b,target))[0];r.puck={x:(actor.x+target.x)/2,y:(actor.y+target.y)/2};r.lastPass=null;r.hockey.pp=null;if(blocker&&rinkDistance(blocker,r.puck)<5)rinkLose(blocker,`${blocker.name} läser och bryter passningen.`);else{r.carrier=null;r.hockey.loose={type:'pass',side:r.owner};rinkSay(`${actor.name}s passning går inte fram. Kamp om en lös puck.`,'turnover');}}
