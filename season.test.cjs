...
run("ensureLeagueStatistics();state.leagueStatistics.rows={};globalThis.awardP=managerRoster().find(p=>p.pos!=='MV');globalThis.awardG=managerRoster().find(p=>p.pos==='MV');state.leagueStatistics.rows.a={id:awardP.id,name:awardP.name,pos:awardP.pos,club:managerClub(),league:'SHL',stage:'regular',games:52,goals:30,assists:40,shots:180,pim:10,seconds:50000,saves:0,against:0,shutouts:0};state.leagueStatistics.rows.g={id:awardG.id,name:awardG.name,pos:'MV',club:managerClub(),league:'SHL',stage:'regular',games:40,goals:0,assists:0,shots:0,pim:0,seconds:140000,saves:1100,against:80,shutouts:5};globalThis.awards=seasonAwards()");
assert.ok(run("awards.some(a=>a.title==='Årets spelare'&&samePlayerId(a.id,awardP.id))"));assert.ok(run("awards.some(a=>a.title==='Årets målvakt'&&samePlayerId(a.id,awardG.id))"));assert.ok(run("awardP.honours.some(h=>h.title==='Årets spelare')"));
run("globalThis.awardHtml=seasonAwardsShowcase([{id:managerRoster()[0].id,name:managerRoster()[0].name,club:managerClub(),title:'Årets spelare',detail:'Testmerit'}])");
assert.match(run("awardHtml"),/Årets spelare/);assert.match(run("awardHtml"),/Testmerit/);
