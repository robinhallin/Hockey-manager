...
function managerDeskView(){
  ...
  return `<section class="manager-desk">
    ...
    ${coachFocus()?`<section class="desk-panel"><h2>Lagets träningsfokus</h2><p>${COACH_FOCUSES[coachFocus().key].name} · ${coachFocus().sessions.length} genomförda pass · ${coachFocus().results.length}/3 uppföljningsmatcher</p>${deskLink('Följ upp fokus',{page:'training'})}</section>`:''}${storiesDeskView()}${rivalsDeskView()}
