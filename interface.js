...
function deskRestore(previous){
 ...
 if(previous.lockerUI)Object.assign(lockerUI,previous.lockerUI);
 if(previous.squadUI)Object.assign(squadUI,previous.squadUI);
...
function deskNavigate(page,tab,record=true){
 ...
 if(record&&page!==previousPage)worldUI.drawer=null;
...
function deskAction(action){if(action.deal)return `officeOpenDeal(${JSON.stringify(action.deal)})`;if(action.contactId!==undefined)return `officeOpenContact(${JSON.stringify(action.contactId)})`;return action.messageId!==undefined?`openManagerMessage(${JSON.stringify(action.messageId)})`:`deskNavigate(${JSON.stringify(action.page)}${action.tab?','+JSON.stringify(action.tab):''})`;}
