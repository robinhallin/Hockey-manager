...
for(const tab of run('DESK_RECRUIT_MORE.map(t=>t[0])')){run(`deskNavigate('transfers','${tab}')`);assert.match(get('#content').innerHTML,/aria-current="page"[^>]*>(Bud & avtal|Spelarsök & scouting)/);assert.doesNotMatch(get('#content').innerHTML,/undefined|NaN/);}
run('deskNavigate("scouting")');assert.match(get('#content').innerHTML,/aria-current="page"[^>]*>Spelarsök & scouting/);
