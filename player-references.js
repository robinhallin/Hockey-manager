...
function playerPerformanceView(id){
 ...
 (records.length?'<div class="workspace-table"><table><thead><tr><th>Match / rapport</th><th>Representerad klubb</th><th>Sammanhang</th><th>Istid</th><th>Mål + assist</th><th>Skott</th><th>Matchbetyg</th></tr></thead><tbody>'+records.map(({match:m,row:r,performance:p})=>{
  ...
  return '<tr><td><a class="player-reference" href="#match/'+encodeURIComponent(m.id)+'" onclick="'+trainingSafe(action)+'">'+trainingSafe(m.club)+' '+m.own+'–'+m.against+' '+trainingSafe(m.opponent)+'</a><small>'+trainingSafe(m.date||String(m.year))+'</small></td><td>'+clubReference(r.club||m.club,{year:m.year})+'</td><td>'+trainingSafe(String(m.year))+' · '+trainingSafe(m.stage||'Tävlingsfas saknas')+(m.partial?' · Delvis registrerad':'')+'</td><td>'+analysisTime(r.seconds)+'</td><td>'+(r.goals??'—')+' + '+(r.assists??'—')+'</td><td>'+(r.shots??'—')+'</td><td>'+performanceRating(p)+'</td></tr>';
...
function playerHistoryView(id){
 ...
 const player=findPlayerAnywhere(id),events=(state.playerWorld?.events||[]).filter(e=>samePlayerId(e.id,id)),career=careerHistoryPlayer(player||{id,careerEvents:[]}),honours=player?.honours||[];
...
function deskWorkspaceContext(){
 return {matchUI:Object.fromEntries(['tab','target','coachMode','changesMode','editPlayers'].map(k=>[k,matchDesk[k]])),calendar:{month:calendarUI.month,date:calendarUI.date},inbox:{...inboxUI},worldLeague:state.world?.selected,
...
function deskRestoreWorkspace(context){
 ...
 Object.assign(matchDesk,context.matchUI||{});
