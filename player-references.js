...
function matchEventReference(e){
 if(e.type==='goal'&&e.scorerId!=null)return playerReference(e.scorerId,e.scorer||'Målskytt')+(Number.isFinite(e.own)&&Number.isFinite(e.against)?' · '+e.own+'–'+e.against:'')+(e.assists?.length?' · Assist: '+e.assists.map(p=>playerReference(p.id,p.name)).join(', '):'');
