...
function managerWeekOpponent(next=deskFixtures().upcoming[0]){
 ...
 if(totals.opp.entries>=totals.own.entries+4)strengths.push('Motståndaren har skapat fler registrerade zoninträden i tidigare möten.');
 if(totals.own.turnovers>=totals.opp.turnovers+4)strengths.push('Deras press har sammanfallit med fler pucktapp för oss.');
 if(totals.opp.battles>=6&&totals.opp.battleWins/totals.opp.battles>=.6)strengths.push('De har vunnit en hög andel registrerade puckdueller.');
 if(totals.opp.turnovers>=totals.own.turnovers+4)weaknesses.push('De har själva haft fler pucktapp i vårt registrerade mötesunderlag.');
 if(totals.own.entries>=totals.opp.entries+4)weaknesses.push('Vi har tidigare tagit oss in i offensiv zon oftare än de.');
 const advice=strengths.some(x=>/pucktapp/.test(x))?'Ge puckföraren tydligt understöd och överväg enklare förstapass när deras press kommer.':
  strengths.some(x=>/zoninträden/.test(x))?'Skydda neutralzonen och bestäm om ni ska störa tidigare eller försvara blålinjen.':
  ({pressure:'Förbered förstapasset och välj utvilade puckförare.',counter:'Behåll täckning bakom pucken och undvik onödiga centrala pucktapp.',control:'Skydda mitten och stäng passningsalternativen.'}[style]||'Underlaget är tunt. Utgå från er egen spelidé.');
 return {next,style,observed,games,reports:reports.length,strengths,weaknesses,advice};
}
