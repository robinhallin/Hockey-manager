...
fresh();run(`state.live=null;useHistoricalCalendarFixture();calendarInitialPreseason();calendarBookFriendly('AIK',calAdd(state.calendar.date,3));globalThis.friendly=state.calendar.friendlies.at(-1);state.calendar.date=friendly.date;calendarPlayFriendly(friendly.id);startMatch();globalThis.standings=JSON.stringify(state.teams);globalThis.totalGoals=managerRoster().reduce((s,p)=>s+(p.goals||0),0);globalThis.e=studioEngine();
