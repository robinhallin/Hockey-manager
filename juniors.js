...
function ensureJuniors(){
 ...
 for(const p of juniorSeasonPlayers())juniorSeasonStart(p,state.juniors.year,true);
}
...
function createJunior(pos,intake){
 ...
 juniorSeasonStart(p,s.year,false);return p;
}
...
function juniorPromote(id){
 ...
 feedbackArrival(p,feedbackPlan,'junior');feedbackNews('promotion:'+p.id+':'+state.calendar.date,managerClub(),'development',playerHeadline(p,' flyttas upp'),managerClub()+' ger junioren en plats i A-truppen.');
 juniorReport(playerHeadline(p," tar steget till A-laget"),`Spelaren får ${careerMoney(p.salary)} per år och ${p.contractYears} års kontrakt. Träningen hjälper, men matchutveckling kräver faktisk istid. Välj plats under Kedjor.`);
...
function juniorAppearance(p,seconds,level,opponent){
 if(internationalAway(p))return {id:p.id,name:p.name,pos:p.pos,seconds:0,goals:0,assists:0};
 const a=p.academy;juniorSeasonStart(p,state.season.year,true);seconds=Math.max(0,Math.min(seconds,medicalLimit(p)));if(!medicalReady(p))seconds=0;
 ...
 if(a.loan)juniorSeasonRecord(p,'loan',{seconds,goals,assists},`${state.season.year}:${state.calendar.date}:${state.round}:${opponent}`);
...
function juniorFixture(key){
 ...
 if(playable){const goals=rows.reduce((n,p)=>n+p.goals,0);let assistsLeft=goals*2;
  ...
  s.matches.unshift({year:s.year,club:managerClub(),round:state.round,opponent,own:goals,against:Math.floor(juniorRoll()*6),players:rows});s.matches=s.matches.slice(0,16);}
...
function juniorNewYear(){
 ...
 for(const p of juniorPlayers()){p.academy.baseline={...p.attributes};p.academy.games=0;p.academy.seconds=0;p.academy.goals=0;p.academy.assists=0;p.academy.missed=0;juniorSeasonStart(p,year,false);}
 ...
 s.intakes.unshift({year,names:incoming.map(p=>p.name),players:incoming.map(p=>({id:p.id,name:p.name}))});s.intakes=s.intakes.slice(0,10);
...
function juniorsView(){return developmentJuniorsView();}
function legacyJuniorsView(){
...
function juniorProfile(p){
 ...
 return `<article class="junior-profile"><header><span class="career-eyebrow">${p.pos} · ${p.age} ÅR · FIKTIV SPELARE</span><h2>${playerReference(p.id,p.name)}</h2><p>${JUNIOR_PATHS[a.path]}${a.loan?` · ${JUNIOR_LOANS[a.loan.destination].name} · ${a.loan.remaining} omgångar kvar`:''}</p></header>${internationalProfile(p)}${nhlProfile(p)}<div class="junior-evaluation"><label>Bedömare<select onchange="state.juniors.assessor=this.value;save();render()">${state.staff.map(q=>`<option value="${q.id}" ${q.id===r.staff.id?'selected':''}>${trainingSafe(q.name)}</option>`).join('')}</select></label><div><span>Förmåga i vald roll</span><strong>${r.current}</strong></div><div><span>Bedömd potential</span><strong>${r.potential}</strong></div><p>${r.confidence} kännedom · ${a.observations} observationer. Stjärnor 0–5 relativt ditt A-lag. Potentialen är osäker och kan bedömas olika av personalen.</p></div><div class="junior-advice"><h3>Junioransvarigs råd</h3><p>${trainingSafe(juniorAdvice(p))}</p></div><h3>Utvecklingsplan</h3><div class="junior-controls"><label>Individuell träning<select onchange="juniorSet('${p.id}','load',this.value)">${[['normal','Normal'],['light','Lätt'],['rest','Vila från träning']].map(([v,label])=>`<option value="${v}" ${p.trainingLoad===v?'selected':''}>${label}</option>`).join('')}</select></label><label>Spelarroll<select onchange="juniorSet('${p.id}','role',this.value)">${juniorRoles(p).map(role=>`<option ${a.role===role?'selected':''}>${role}</option>`).join('')}</select></label><label>Mentor<select onchange="juniorSet('${p.id}','mentor',this.value)"><option value="">Ingen mentor</option>${mentors.map(m=>`<option value="${m.id}" ${samePlayerId(a.mentor,m.id)?'selected':''}>${trainingSafe(m.name)} · ${m.age} år</option>`).join('')}</select></label>${!own&&!a.loan?`<label>Träningsmiljö<select onchange="juniorSet('${p.id}','path',this.value)">${['junior','guest'].map(path=>`<option value="${path}" ${a.path===path?'selected':''}>${JUNIOR_PATHS[path]}</option>`).join('')}</select></label>`:''}</div><p>Mentorskap tränar arbetsvanor eller kyla och stärker tryggheten under genomförda pass. Högst två talanger per mentor. ${mentor?`${playerReference(mentor.id,mentor.name)} kan ge stöd.`:a.mentor?'Mentorn är frånvarande eller saknar möjlighet att delta just nu.':'Välj en erfaren spelare som stöd.'} På lån sköter den mottagande klubben träningen.</p><details class="junior-next" ${juniorNextOpen?'open':''} ontoggle="juniorNextOpen=this.open"><summary>Byt utvecklingsmiljö · A-lag eller lån</summary><div class="junior-actions">${own?`<button class="btn secondary" onclick="selectPlayer('${p.id}')">A-lagsprofil</button>${p.age<=20?`<button class="btn secondary" onclick="juniorReturn('${p.id}')">Tillbaka till juniorlaget</button>`:''}`:a.loan?`<button class="btn secondary" onclick="juniorRecall('${p.id}')">Återkalla lån</button>`:`<button class="btn" onclick="juniorPromote('${p.id}')">Flytta upp · ${careerMoney(p.salary)}/år</button>${p.age>20?`<button class="btn secondary" onclick="juniorRelease('${p.id}')">Släpp · ${careerMoney(a.seniorContract?p.salary*p.contractYears:0)}</button>`:''}`}</div>${!own&&!a.loan?`<p>Uppflyttning ger ${p.contractYears||3} års A-avtal. Juniorträning och utvecklingsmatcher ingår i klubbens verksamhet; ett tecknat A-avtal fortsätter belasta lönebudgeten även efter återgång till juniorlaget.</p><div class="junior-loans">${Object.entries(JUNIOR_LOANS).map(([key,offer])=>`<article><h4>${offer.name}</h4><p>${offer.description}. Åtta matchomgångar, inga låneavgifter. ${p.pos==='MV'?'Rotation mellan målvakter.':`Riktmärke ${offer.minutes} min, beroende på konkurrens och ork.`}</p><button class="btn secondary" onclick="juniorLoan('${p.id}','${key}')">Acceptera utvecklingslån</button></article>`).join('')}</div>`:''}</details><details class="dv-report"><summary>Attribut & utveckling</summary><div class="junior-attributes">${Object.entries(fields).map(([key,label])=>`<div><span>${label}</span><strong>${p.attributes[key]} <small>${p.attributes[key]>a.baseline[key]?`+${p.attributes[key]-a.baseline[key]}`:''}</small></strong></div>`).join('')}</div><p>Egna spelares attribut på skalan 1–20. Förändring sedan säsongens start eller ankomst. Rollplanen påverkar vilka egenskaper som tränas.</p></details><details class="dv-report"><summary>Senaste matcherna</summary>${a.history.slice(0,6).map(m=>`<div class="junior-stat"><span>${m.opponent}<small>${JUNIOR_PATHS[m.path]}</small></span><span>${Math.floor(m.seconds/60)} min</span><strong>${m.goals}+${m.assists}</strong></div>`).join('')||'<p>Matchvana registreras när matcher spelas.</p>'}</details></article>`;
}
