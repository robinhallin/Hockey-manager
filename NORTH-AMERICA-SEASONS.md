...
`state.naLeagues` holds the current season and at most three archived seasons. Completed seasons use the existing lossless career packer; only champion summaries remain unpacked. Selecting an archived year decodes a read-only view and retains every fixture and player row while keeping the active save below its storage budget. Fixtures have stable IDs and a played marker, so reloads or repeated date processing cannot count a game twice. Each completed match retains goals, team shots, duration, tracked player rows and any injuries. The UI reads these records; navigation and filtering do not simulate matches.
