...
function hockeySpecial(side){const m=state.live,diff=StudioHockey.penaltyCount(m.penaltiesOpp)-StudioHockey.penaltyCount(m.penaltiesHV);return diff===0?'even':(side==='own'?diff:-diff)>0?'pp':'pk';}
...
function hockeyChangeBlocked(side='own'){if(studioActive())return studioEngine().icingHold===(side==='own'?0:1)&&studioEngine().stoppage>0;const r=state.live?.rink;return Boolean(r?.restart&&r.hockey?.icingHold?.side===side);}
