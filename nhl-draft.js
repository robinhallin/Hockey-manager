...
function nhlRunDraft(date=state.calendar.date){
 ...
 for(const origin of new Set(d.picks.map(r=>r.origin).filter(c=>state.world.membership[c]))){const rows=d.picks.filter(r=>r.origin===origin);feedbackNews(`nhl-draft:${d.year}:${origin}`,origin,'development',`${rows.length} spelare från ${origin} valda i NHL-draften`,[...rows.flatMap(r=>[playerMention(r),`: ${r.club}, val ${r.overall}. `]),'NHL-rättigheter registreras; de svenska klubbavtalen fortsätter gälla.']);}
...
function nhlDay(){
 ...
 for(const {p} of internationalPlayers()){
  if(p.nhlDraft&&!naActive(p)&&!p.nhlDraft.expiredNotice&&date>p.nhlDraft.expires){p.nhlDraft.expiredNotice=true;if(nhlOwned(p))managerMessage(`nhl-expiry:${p.id}:${p.nhlDraft.year}`,`${p.name}: NHL-rättigheterna har löpt ut`,`${p.nhlDraft.club} har inte längre exklusiva NHL-rättigheter i spelmodellen. Klubbavtalet här påverkas inte.`,'Sportchefen',{link:'nhl'});}
...
function nhlSetPlan(id,path){
 ...
 if(!p||!w||naActive(p)||!nhlOwned(p)||!nhlRightsActive(p)||!managerEmployed()||state.live&&!state.live.finished||!['senior','junior','open'].includes(path)||p.nhlPlan?.season===state.season.year)return false;
...
function nhlSetFilter(key,value){if(!Object.hasOwn(nhlUI,key))return;if(key==='tab'&&!['club','board','results','rights','contracts','leagues'].includes(value))return;if(key==='club'&&value!=='all'&&!NHL_CLUBS.includes(value))return;if(key==='round'&&!['all','1','2','3','4','5','6','7'].includes(value))return;nhlUI[key]=String(value).slice(0,80);render();}
