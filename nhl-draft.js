...
function nhlSetFilter(key,value){if(!Object.hasOwn(nhlUI,key))return;if(key==='tab'&&!['club','board','results','rights','contracts'].includes(value))return;if(key==='club'&&value!=='all'&&!NHL_CLUBS.includes(value))return;if(key==='round'&&!['all','1','2','3','4','5','6','7'].includes(value))return;nhlUI[key]=String(value).slice(0,80);render();}
...
function nhlView(){
 ...
 const tabs=[['club','Din klubbs talanger'],['board','Scoutprognos'],['results','Draftresultat'],['rights','NHL-rättigheter'],['contracts','Kontrakt & Nordamerika']];
 let body='';
