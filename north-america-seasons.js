...
function nasView(){
 ...
 if(nasUI.tab==='games'){
  const games=l.games.filter(g=>nasUI.club==='all'||g.home===nasUI.club||g.away===nasUI.club),upcoming=games.filter(g=>!g.played).slice(0,24),recent=games.filter(g=>g.played).reverse().slice(0,48);
...
function nasGameView(g){return `<section class="mw-panel"><h2>${trainingSafe(g.home)} ${g.hg}–${g.ag} ${trainingSafe(g.away)}</h2><p>${calText(g.date)} · Skott ${g.shots.join('–')} · ${g.shootout?'Straffavgörande (avgörande lagmål ingår inte i individuell statistik)':g.overtime?'Förlängning':'Ordinarie tid'} · speltid ${Math.floor(g.duration/60)}:${String(g.duration%60).padStart(2,'0')}</p><h3>Mål</h3>${g.events.map(e=>`<p>${Math.floor(e.time/60)}:${String(e.time%60).padStart(2,'0')} · ${trainingSafe(e.side?g.away:g.home)} · ${playerReference(e.id,e.scorer)}${e.assists.length?' ('+e.assists.map(a=>playerReference(a.id,a.name)).join(', ')+')':''}</p>`).join('')||'<p>Inga mål under ordinarie tid eller förlängning.</p>'}<h3>Bevakade spelare</h3>${g.players.map(p=>`<p>${playerReference(p.id,p.name)} · ${trainingSafe(p.club)} · ${Math.round(p.seconds/60)} min · ${p.pos==='MV'?p.saves+' räddningar, '+p.against+' insläppta':p.goals+' mål, '+p.assists+' assist, '+p.shots+' skott'}</p>`).join('')||'<p>Den här matchen spelades helt med lagmodellen.</p>'}${(g.injuries||[]).map(p=>`<p>${playerReference(p.id,p.name)} skadades · bedömd frånvaro ${p.days} dagar vid matchtillfället.</p>`).join('')}</section>`;}
