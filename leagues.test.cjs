...
run('(!state.live&&(state.calendar.date=calendarTarget()),startMatch());for(let i=0;i<100000&&!state.live.finished;i++){if(!state.live.running)(!state.live&&(state.calendar.date=calendarTarget()),startMatch());liveStep()}');
