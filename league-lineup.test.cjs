...
run('(!state.live&&(state.calendar.date=calendarTarget()),startMatch());state.live.rink.restart=true;state.live.rink.hockey.icingHold={side:"own",ids:rinkOwnPlayers().map(p=>p.id)};globalThis.blockedLines=JSON.stringify(state.lines);lineupPickSlot("forwards",0);lineupPlace(state.lines.forwards[1])');assert.equal(run('JSON.stringify(state.lines)'),run('blockedLines'));
