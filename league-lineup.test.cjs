...
const friendly=boot();friendly.run('startCareerWithClub("HV71");calendarInitialPreseason();calendarBookFriendly("AIK",calAdd(state.calendar.date,3));calendarContinue();calendarContinue();calendarContinue();calendarContinue();(!state.live&&(state.calendar.date=calendarTarget()),startMatch());trackIceTime(60);globalThis.p=currentLinePlayers()[0];recordAnalysisShot("own",p.name,p.id,true,{x:80,y:50,factor:1},1,0,"goal");goalHV(p);calendarFinishFriendly()');
