...
assert.equal(run('JSON.stringify(state.lines)'),run('originalLines'));assert.equal((get('#content').innerHTML.match(/class="lineup-slot /g)||[]).length,6);assert.match(get('#content').innerHTML,/onclick="lineupPickSlot\('forwards',6\)"/);assert.match(get('#content').innerHTML,/onclick="lineupPickSlot\('defense',2\)"/);
