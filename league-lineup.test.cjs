...
assert.equal(run('JSON.stringify(state.lines)'),run('originalLines'));assert.equal((get('#content').innerHTML.match(/data-lineup-slot="/g)||[]).length,19);assert.match(get('#content').innerHTML,/onclick="lineupPickSlot\('forwards',6\)"/);assert.match(get('#content').innerHTML,/onclick="lineupPickSlot\('defense',2\)"/);
...
for(const club of run('Object.keys(state.world.membership)')){run(`startCareerWithClub(${JSON.stringify(club)});deskNavigate('lines');lineupPickSlot('forwards',0)`);assert.doesNotMatch(get('#content').innerHTML,/undefined|NaN/);assert.match(get('#content').innerHTML,/Spelarval och jämförelse/);assert.match(get('#content').innerHTML,/aria-label="Välj /);}
