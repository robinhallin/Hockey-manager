...
const so=boot();so.run('startCareerWithClub("HV71");(!state.live&&(state.calendar.date=calendarTarget()),startMatch());globalThis.goalie=leagueKeeper("own"),enemyGoalie=leagueKeeper("opponent");leagueLivePlayer("own",goalie.id).seconds=3900;leagueLivePlayer("opponent",enemyGoalie.id).seconds=3900;state.live.period=4;state.live.minute=5;state.live.second=0;shootout()');
...
const friendly=boot();friendly.run('startCareerWithClub("HV71");useHistoricalCalendarFixture();calendarInitialPreseason();calendarBookFriendly("AIK",calAdd(state.calendar.date,3));calendarContinue();calendarContinue();calendarContinue();calendarContinue();(!state.live&&(state.calendar.date=calendarTarget()),startMatch());trackIceTime(60);globalThis.p=currentLinePlayers()[0];recordAnalysisShot("own",p.name,p.id,true,{x:80,y:50,factor:1},1,0,"goal");goalHV(p);calendarFinishFriendly()');
...
assert.equal(run('JSON.stringify(state.lines)'),run('originalLines'));
for(const [type,count] of [['forwards',12],['defense',6],['goalie',1],['backup',1]])assert.equal((get('#content').innerHTML.match(new RegExp('data-lineup-slot="'+type+'-','g'))||[]).length,count);
assert.match(get('#content').innerHTML,/onclick="tacticsPick\('forwards',6,true\)"/);assert.match(get('#content').innerHTML,/onclick="tacticsPick\('defense',2,true\)"/);
...
run('(!state.live&&(state.calendar.date=calendarTarget()),startMatch());studioEngine().icingHold=0;studioEngine().stoppage=2;globalThis.blockedLines=JSON.stringify(state.lines);lineupPickSlot("forwards",0);lineupPlace(state.lines.forwards[1])');assert.equal(run('JSON.stringify(state.lines)'),run('blockedLines'));
run('studioEngine().icingHold=null;save()');const formation=boot(app.storage.value);assert.equal(formation.run('JSON.stringify(state.lines)'),run('JSON.stringify(state.lines)'));
...
for(const club of run('Object.keys(state.world.membership)')){run(`startCareerWithClub(${JSON.stringify(club)});deskNavigate('lines');lineupPickSlot('forwards',0)`);assert.doesNotMatch(get('#content').innerHTML,/undefined|NaN/);assert.match(get('#content').innerHTML,/aria-label="Spelarval"/);assert.match(get('#content').innerHTML,/aria-label="Välj /);}
