...
function seasonView(){
 ...
 ${s.phase==='review'?(()=>{const review=seasonReviewPackage(latest||{});return `<section class="season-review"><h2>Säsongsavslutning</h2><h3>Styrelsens utvärdering</h3>${boardGoalsHTML(s.boardResult,true)}<h3>SHL-utmärkelser</h3>${review.awards.map(a=>`<div class="row"><span>${trainingSafe(a.title)} · ${playerReference(a.id,a.name)}</span><strong>${trainingSafe(a.club)} · ${trainingSafe(a.detail)}</strong></div>`).join('')||'<p>Inga fullständiga statistikunderlag för utmärkelser.</p>'}<h3>Inför sommaren</h3><div class="career-finances"><div><span>Utgående kontrakt</span><strong>${review.expired.length}</strong></div><div><span>Klubbprojekt</span><strong>${trainingSafe(review.project.name)} · ${review.project.maturity}%</strong></div><div><span>Spelarvärlden</span><strong>${review.world?review.world.retired+' pensioneringar':'Avslutas vid säsongsskiftet'}</strong></div></div>${review.movement?`<p>Serieomflyttning: ${trainingSafe(JSON.stringify(review.movement))}</p>`:''}${review.project.milestone?`<p>Projektets valda milstolpe: <strong>${trainingSafe(review.project.milestone)}</strong>.</p>`:''}<button class="btn" onclick="beginPreseason()">Inled offseason ${seasonLabel(s.year+1)} →</button></section>`})():''}
