"use strict";
const SEASON_STAGES={playin:'Åttondelsfinal',quarter:'Kvartsfinal',semi:'Semifinal',final:'Final',playout:'SHL-kval',playinFinal:'Play-in: sista platsen'};
...
function regularTable(){return leagueTable();}
...
function seasonRank(name){return (Object.values(state.world?.tables||{}).find(rows=>rows.some(t=>t.name===name))||(state.season.standings?.some(t=>t.name===name)?state.season.standings:leagueTable(leagueOf(name)))).findIndex(t=>t.name===name)+1;}
function addSeries(stage,names,league=leagueOf()){
 const s=state.season,ordered=[...names].sort((a,b)=>seasonRank(a)-seasonRank(b));s.stage=stage;
 for(let i=0;i<ordered.length/2;i++)s.series.push({id:`${s.year}-${league}-${stage}-${i}`,league,stage,high:ordered[i],low:ordered[ordered.length-1-i],winsHigh:0,winsLow:0,bestOf:stage==='playin'?3:7,games:[],winner:null});
}
...
function enterPlayoffs(){
 ...
 if(!leagueStartPlayoffs()){addSeries('playin',s.standings.slice(6,10).map(t=>t.name));schedulePlayoffDay();}
...
function recordSeriesGame(game,homeGoals,awayGoals){
 if(aggregateSeries(game))return aggregateRecord(game,homeGoals,awayGoals);
...
function finishPlayoffDay(){
 if(!calendarToMatch())return false;
 ...
 if(!calendarToMatch())return false;state.round++;calendarAfterFixture();advancePlayoffStage();return true;
}
function finishPlayoffMatch(){
 const m=state.live,g=currentSeasonFixture();if(!m||m.finished||!g||(!aggregateSeries(g)&&m.hv===m.opp))return;
...
function seasonAwards(){
 const rows=seasonLeagueRows(swissCareer()?'CH_NL':'SHL','regular'),skaters=rows.filter(r=>r.pos!=='MV'&&r.games>=10),goalies=rows.filter(r=>r.pos==='MV'&&r.games>=8);
 ...
 const recordLeague=swissCareer()?'nl':'shl',recordLabel=swissCareer()?'National League':'SHL';
 for(const r of skaters){careerRecord(`${recordLeague}-career-games`,`Flest registrerade ${recordLabel}-matcher`,r.games,r.name,{playerId:r.id,club:r.clubs[0]});careerRecord(`${recordLeague}-season-points`,`Flest ${recordLabel}-poäng under en säsong`,r.points,r.name,{playerId:r.id,club:r.clubs[0]});careerRecord(`${recordLeague}-season-goals`,`Flest ${recordLabel}-mål under en säsong`,r.goals,r.name,{playerId:r.id,club:r.clubs[0]});}
 return awards;
}
function seasonRecordHighlights(record){
 const current=record?.records||{},archive=state.season.archive?.[0]?.records||{},rows=Object.values(current).filter(r=>r?.year===state.season.year||r?.date?.startsWith?.(String(state.season.year))).slice(0,6);
 return rows.length?rows:Object.values(current).filter(r=>['shl-season-points','shl-season-goals','manager-season-points','manager-season-goals'].includes(r.key)).slice(0,6);
}
function seasonAwardsShowcase(awards){
 return `<section class="season-awards-showcase"><span class="career-eyebrow">${swissCareer()?'NATIONAL LEAGUE':'SHL'} AWARDS · ${seasonLabel()}</span><h2>Säsongens utmärkelser</h2><div class="season-awards-grid">${(awards||[]).map(a=>`<article><small>${trainingSafe(a.title)}</small><h3>${playerReference(a.id,a.name)}</h3><p>${trainingSafe(a.club)} · ${trainingSafe(a.detail)}</p></article>`).join('')||'<p>Inga fullständiga statistikunderlag för utmärkelser.</p>'}</div></section>`;
}
...
function watchRemainingPlayoffs(){
 if(state.season?.phase!=='playoffs'||currentSeasonFixture())return;
 calendarContinue();
}
...
function launchSeason(){
 ...
 if(!calendarLaunch()){state.page='calendar';calendarNotify('Fortsätt en dag i taget. Grundserien kan startas från 7 september.');return;}
 ...
 state.teams=leagueTeamRows(state.world.membership).map(([name,strength,style])=>({name,strength:Math.round(state.clubRosters[name].reduce((n,p)=>n+matchAttributeRating(p),0)/state.clubRosters[name].length),style,gp:0,w:0,l:0,otw:0,otl:0,pts:0,gf:0,ga:0}));
 state.managerCareer.startGames=0;state.managerCareer.lastReview=null;
 state.schedule=createSchedule(state.world.membership);leagueReset();state.round=1;state.selectedRound=1;state.live=null;
 ...
 ensureTrainingData();if(oldTraining){state.training.plan=oldTraining.plan;state.training.familiarity=oldTraining.familiarity;for(const key of ['recoveryOwner','assistantOwner','reporting','history','messages','nextMessageId'])if(oldTraining[key]!=null)state.training[key]=oldTraining[key];}
...
function seasonMatchPanel(){
 ...
 const series=state.season.series.find(s=>s.id===g.seriesId);
 return `<section class="season-banner"><span>${SEASON_STAGES[g.stage]} · MATCH ${series.games.length+1} · ${seriesFormat(series)}</span><h2>${series.high} <b>${series.winsHigh}–${series.winsLow}</b> ${series.low}</h2><p>${g.home===managerClub()?'Hemmaplan':'Bortaplan'} · ${series.aggregate?'Första matchen slutar efter 60 minuter. Returen får sudden death bara vid lika sammanlagt resultat.':'Förlängning avgörs med sudden death, utan straffläggning.'}</p><button class="btn secondary" onclick="trainingOpen('training')">Återhämtning & matchplanering</button></section>`;
}
function careerRecordsView(){
 const h=ensureCareerHistory(),rows=Object.values(h.records||{}).sort((a,b)=>b.value-a.value);
 return `<section class="season-archive"><h2>Karriärrekord</h2>${rows.map(r=>`<div class="row"><span>${trainingSafe(r.label)}</span><strong>${trainingSafe(r.holder)} · ${Number(r.value).toLocaleString('sv-SE')}${r.club?' · '+trainingSafe(r.club):''}</strong></div>`).join('')||'<p>Rekord börjar registreras när karriären producerar nya milstolpar.</p>'}<h3>Senaste historiska händelser</h3>${h.events.slice(0,12).map(e=>`<p>${e.date?calText(e.date):seasonLabel(e.year)} · ${trainingSafe(e.title)}${e.detail?' · '+trainingSafe(e.detail):''}</p>`).join('')||'<p>Inga händelser registrerade ännu.</p>'}</section>`;
}
...
function seasonView(){
 if(preseasonPlan()?.pending)return preseasonPlanningView();
 if(state.live?.delegatedFriendly&&!state.live.finished)return preseasonProgressView();
 ...
 ${series.length?`<div class="season-bracket">${Object.entries(SEASON_STAGES).map(([stage,name])=>`<section><h2>${name}</h2>${series.filter(x=>x.stage===stage).map(x=>`<article class="series-card ${[x.high,x.low].includes(managerClub())?'own-series':''}"><div><strong>${x.high}</strong><b>${x.winsHigh}</b></div><div><strong>${x.low}</strong><b>${x.winsLow}</b></div><p>${x.winner?`${x.winner} ${x.stage==='final'?'är mästare':'vidare'}`:seriesFormat(x)}</p><details><summary>Matcher</summary>${x.games.map(g=>`<p>${g.home} ${g.homeGoals}–${g.awayGoals} ${g.away}</p>`).join('')||'<p>Ingen match spelad.</p>'}</details></article>`).join('')||'<p>Avgörs efter föregående omgång.</p>'}</section>`).join('')}</div>`:''}
 ${s.phase==='review'?(()=>{const review=seasonReviewPackage(latest||{});return `<section class="season-review"><h2>Säsongsavslutning</h2>${seasonAwardsShowcase(review.awards)}<h3>Styrelsens utvärdering</h3>${boardGoalsHTML(s.boardResult,true)}<h3>Inför sommaren</h3><div class="career-finances"><div><span>Utgående kontrakt</span><strong>${review.expired.length}</strong></div><div><span>Klubbprojekt</span><strong>${trainingSafe(review.project.name)} · ${review.project.maturity}%</strong></div><div><span>Spelarvärlden</span><strong>${review.world?review.world.retired+' pensioneringar':'Avslutas vid säsongsskiftet'}</strong></div></div>${review.movement?`<p>Serieomflyttning: ${trainingSafe(JSON.stringify(review.movement))}</p>`:''}${review.project.milestone?`<p>Projektets valda milstolpe: <strong>${trainingSafe(review.project.milestone)}</strong>.</p>`:''}<details><summary>Rekordbild efter säsongen</summary>${careerRecordsView()}</details><button class="btn" onclick="beginPreseason()">Inled offseason ${seasonLabel(s.year+1)} →</button></section>`})():''}
 ${s.phase==='preseason'?preseasonDeskView()+preseasonView():preseasonPlan()?.report?preseasonDeskView():''}
 <details class="league-season-archive"><summary>Tidigare säsonger & karriärhistorik</summary>${careerHistoryView()}${careerRecordsView()}<section class="season-archive"><h2>Karriärhistorik</h2>${s.archive.map(a=>`<details><summary>${seasonLabel(a.year)} · Mästare: ${a.champion} · ${a.club}, plats ${a.position}</summary><p>Styrelsens mål: ${a.goals.filter(g=>g.met).length}/${a.goals.length} uppnådda. Spelarstatistiken nedan omfattar hela säsongen inklusive slutspel.</p>${a.awards?.length?`<h3>Utmärkelser</h3>${a.awards.map(x=>`<div class="row"><span>${trainingSafe(x.title)}</span><strong>${trainingSafe(x.name)} · ${trainingSafe(x.club)}</strong></div>`).join('')}`:''}${[...a.players].sort((p,q)=>(q.goals+q.assists)-(p.goals+p.assists)).map(p=>`<div class="row"><span>${p.name}</span><strong>${p.goals} mål · ${p.assists} assist · ${p.development} attributsteg</strong></div>`).join('')}<h3>Sluttabell</h3>${a.standings.map((t,i)=>`<div class="row"><span>${i+1}. ${t.name}</span><b>${t.pts} p</b></div>`).join('')}</details>`).join('')||'<p>Avslutade säsonger sparas här.</p>'}</section></details><p class="training-note">${swissCareer()?swissRulesText():'SHL och Hockeyallsvenskan spelar parallellt. Allsvenska mästaren går upp; förloraren i SHL-kvalet går ned. Se Ligavärlden för båda tabellerna.'}</p></section>`;
}
function preseasonView(){const s=state.season,expired=managerRoster().filter(p=>p.contractYears<=0);return `<section class="season-review"><h2>Försäsong ${seasonLabel()}</h2><div class="career-finances"><div><span>Ny tilldelning</span><strong>${careerMoney(s.grant)}</strong></div><div><span>Ny lönebudget</span><strong>${careerMoney(s.nextWageLimit)}</strong></div></div><p>Nuvarande årslöner: ${careerMoney(annualWageCost())}. ${annualWageCost()>s.nextWageLimit?'Du ligger över lönebudgeten. Du kan starta säsongen, men styrelsens ekonomimål kräver att lönerna minskar.':''} ${s.archive.length?'Spelarna har blivit ett år äldre. ':''}Utgående kontrakt behöver ditt beslut.</p>${expired.map(p=>`<div class="preseason-player"><strong>${p.name} · ${p.pos}</strong><div><button class="btn secondary" onclick="preseasonRenew('${p.id}')">Förhandla nytt avtal</button><button class="btn secondary" onclick="releaseExpiredPlayer('${p.id}')">Avsluta avtalet</button></div></div>`).join('')||'<p>Alla spelare har giltiga kontrakt.</p>'}<h3>Flytta upp juniorer</h3><p>Treårsavtal, 350 000 kr per år. Juniorerna är skapade spelare.</p><div class="training-presets">${['MV','B','C','VF','HF'].map(pos=>`<button onclick="recruitAcademyPlayer('${pos}')">+ ${pos}</button>`).join('')}</div><h3>Gemensam marknad för kontraktslösa</h3>${s.freeAgents.slice(0,12).map(p=>`<div class="preseason-player"><span>${p.name} · ${p.pos}</span><button class="btn secondary" onclick="signSeasonFreeAgent('${p.id}')">Förhandla avtal</button></div>`).join('')||'<p>Inga spelare i listan.</p>'}<button class="btn secondary" onclick="trainingOpen('transfers')">Öppna rekrytering</button><p role="status">${s.message||''}</p><p>Grundserien kan startas från 7 september. Fram till dess följer du lagets dagsprogram i kalendern.</p><button class="btn" onclick="${state.calendar.date<state.season.year+'-09-07'?"deskNavigate('calendar')":'launchSeason()'}">${state.calendar.date<state.season.year+'-09-07'?'Till dagens program':'Godkänn truppen och starta grundserien'} →</button></section>`;}
