...
function naAssignTeam(team,date=state.calendar.date){
 ...
 rows.forEach((p,i)=>{const c=p.naContract,level=naAbility(p)>=14.5&&i<({MV:1,B:2,F:3})[group]?'NHL':'AHL';if(c.assignment!==level){c.assignment=level;naLog(p,'assignment',`${level==='NHL'?'Uppflyttning / plats i':'Utvecklingsplacering i'} ${level==='NHL'?team:NA_AFFILIATES[team]}. Bedömning av nuvarande egenskaper och konkurrens i den bevakade gruppen.`,date);if(c.homeClub===managerClub())managerMessage(`na-place:${p.id}:${date}:${level}`,`${p.name}: ${level}`,`Spelaren finns nu hos ${naLocation(p)}. Placeringen grundas på egenskaper och konkurrens; inga spelade NHL/AHL-matcher påstås.`,'Utlandsbevakningen',{link:'nhl'});}p.club=naLocation(p);p.salary=level==='NHL'?c.nhlSalary:c.ahlSalary;});}
}
...
function naBorrow(id){
 ...
 if(!p||!managerEmployed()||naLocked()||!calendarWindowOpen()||!naActive(p)||p.naContract.assignment!=='AHL'||p.naContract.end<state.calendar.date||internationalAway(p)||!medicalReady(p)||!loanFit(p,managerClub()).interested)return false;
...
function naDay(){
 ...
 for(const p of w.abroad){
  ...
  if(calGap(c.lastReport,date)>=28){c.lastReport=date;if(c.homeClub===managerClub())managerMessage(`na-report:${p.id}:${date}`,`Utlandsrapport: ${p.name}`,`${naLocation(p)} · ${c.trainingDays} registrerade utvecklingspass sedan avtalets början. ${p.health.injury?'Rehabilitering pågår.':'Tränar med sin placerade grupp.'} NHL/AHL-matchstatistik simuleras ännu inte i denna etapp.`,'Utlandsbevakningen',{link:'nhl'});}
 }
