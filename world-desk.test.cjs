...
run("ensureRivals();globalThis.newsRows=worldHockeyNews()");
assert.ok(run("Array.isArray(newsRows)&&newsRows.length<=20"));assert.doesNotThrow(()=>run("worldHockeyNewsView()"));
