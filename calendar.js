...
function calendarAgenda(date){
 ...
 ${!matches.length?(log?trainingDecisionReport(log):editable?trainingCalendarDecision(date,session):''):''}
...
function calendarFutureRoom(club=managerClub(),excludePlayerId=null){
 ...
 const pending=state.recruitment.deals.filter(d=>d.status==='pending'&&(excludePlayerId===null||!samePlayerId(d.playerId,excludePlayerId))&&(d.kind==='future'?d.buyer===club:club===managerClub()&&(!d.buyer||d.buyer===club)&&d.years>1)).reduce((n,d)=>n+d.salary,0);
...
function submitFutureOffer(id,salary,years,role){
 ...
 if(!p||naActive(p)||playerLoan(p)||isOwnPlayer(p)||p.contractYears!==1||p.futureContract||state.season.phase==='preseason')return recruitMessage('Förhandsavtal gäller spelare i andra klubbar med ett kontraktsår kvar, under pågående säsong.');
...
function calendarActivateFuture(){
 ...
 for(const p of [...Object.values(state.clubRosters).flat(),...(state.playerWorld?.freeAgents||[])]){
  ...
  trainingClubChange(p,seller,f.buyer);
