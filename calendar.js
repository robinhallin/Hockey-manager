...
function calendarAgenda(date){
 ...
 ${decisions.map(m=>`<button class="cal-task" onclick="openManagerMessage(${m.id})"><span>SVAR BEHÖVS</span><strong>${trainingSafe(m.title)}</strong><small>Besvara innan dagen går vidare →</small></button>`).join('')}
 ...
 ${today?`<p class="cal-advice">${trainingSafe(trainingAdvice())}</p><button class="btn cal-continue" onclick="calendarContinue()">${calendarActionLabel()}</button><p class="cal-hint">En dag i taget. Rapporterna finns kvar i inkorgen; samtal som kräver svar stoppar tiden.</p>`:''}
