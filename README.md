...
## League statistics, standings and rink lineup selection

Ligorna now contains a full standings view and league-wide player statistics for SHL and Hockeyallsvenskan. Standings show regulation/OT wins and losses, goals for/against, goal difference, points per game and the last five results. Club names open their player lists. Player lists include points, goals, assists, shots on target, penalty minutes and goalie appearances, ice time, saves, goals against, save percentage, GAA and shutouts. Columns can be sorted, and filters cover league, club, competition, player name and minimum appearances. Tables show 50 rows at a time. Regular season and playoffs/qualification have separate ledgers; the two most recent prior seasons remain selectable.

The new `league-statistics.js` ledger records both sides of live games and generates player box scores for all background fixtures. AI statistics use an independent deterministic generator, with shots and assists weighted by actual player attributes, and do not consume the match engine's random stream. Shots count goals, saves and rebounds, excluding misses, posts and blocks. Saves/goals against belong to the keeper actually in goal; empty-net and shootout goals do not count against a goalie. Minutes are tracked separately for each participating goalie. Completed games are committed once. Club stints stay attached to their original league/club after transfers. Exhibition games are excluded. Existing player-profile totals remain compatible.

Older saves do not contain full AI box scores. Their missing historical games are identified and the league lists start at the update, without invented historical scorers or reconstructed goalie rates. An already-running match is marked partial. The full standings retain earlier results. Regular/player stats are not retroactively mixed with playoffs, and partial/administratively ended matches cannot award shutouts.

`lineup-board.js` replaces dropdown stacks with a rink containing six interactive player slots, independent selectors for four forward lines and three defense pairs, candidate cards, a goalie picker and an overview of the entire lineup. Selecting an already-used skater swaps the two places. Position, injury and icing restrictions still apply; no duplicate skaters are introduced. Selecting a formation for preview does not change the active match unit. Player changes use the existing match-engine lineup. The squad view displays both ability and potential and becomes compact player cards on narrow screens.

Staff and junior reports use five actual star symbols, including clipped half stars. Ability is gold; potential is blue. Solid stars represent the lower bound and translucent stars the possible upper range. Tooltips/screen-reader labels preserve the numeric uncertainty and assessor identity. Plain native selection options use star symbols rather than numeric star ranges. Attribute and scouting calculations are unchanged.

Validation: 19 existing Node suites pass. `league-lineup.test.cjs` covers AI box-score conservation and RNG isolation, live goalie accounting/empty nets, all 28 teams, sorting/filters, transfers, season archives, legacy migration, a completed 2D match, playoffs, shootouts, friendly isolation, lineup swaps/goalies/icing, saved selections and coloured half-star markup. Generated HTML and event-handler syntax checked. Visual browser QA has not been performed. Release cache version: `league1`.


## Levande motståndarklubbar (rivals1)

Alla 28 svenska klubbar har nu en sparad tränarprofil. Motståndarnas fiktiva tränare har en spelidé, kedjeanvändning, ungdomsprioritering, anpassningsförmåga och stöd från styrelsen. Svaga resultat kan leda till en ny matchplan och, efter minst tio matcher på posten, ett tränarbyte. Högst två byten per klubb och säsong. Tränarhistoriken följer med till nästa säsong; användarens befintliga karriärsystem styr fortsatt den egna anställningen.

- Matcher → Motståndsrapport visar tränare, form, skott, powerplay, trolig målvakt, skadeavbräck, förväntade kedjor och tidigare inbördes möten. Prognoserna använder samma uttagning som nästa match. Tränarkontoret visar en kompakt rapport och aktuella avbräck inför nästa motståndare.
- SHL, Hockeyallsvenskan och bakgrundsslutspelet använder samma lätta händelsesimulering. Spelklarhet, individuella attribut, belastning, laguttagning, tränarstil och samspel påverkar den. Skott, mål, assist, utvisningar, istid och målvaktsstatistik skapas tillsammans med resultatet. Ett avgörande straffmål räknas endast i lagresultatet.
- Den direktsända 2D-matchen använder motståndarnas sparade laguttagning, målvaktsval och egna rotation. Motståndaren kan höja pressen, försvara en ledning och ta timeout. Målvaktsval och skyttens egna attribut används också när skott avgörs. Skadade motståndare väljs bort.
- Motståndarnas belastning och form består mellan matcherna. Kalenderdagar ger återhämtning, träning och en liten skaderisk; matchdeltagande ger belastning, utvecklingsarbete och skaderisk. Den tidigare separata årliga attributökningen för AI-spelare har tagits bort för att undvika dubbla utvecklingssystem.
- Säsongshistorien Duellen på tränarbänken kan börja vid ett tränarbyte inför ett kommande möte eller efter två nederlag mot samma tränare. Två riktiga förberedelsepass eller bibehållen matchplan ger ett litet stöd i den berörda matchen. Historien bestämmer aldrig resultatet och ger inga automatiska spelarvärvningar, uppställningar eller segrar.
