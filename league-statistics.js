...
function ensureLeagueStatistics(){
 ...
 if(s.year!==year){
  s.archives.unshift({year:s.year,rows:s.rows,recorded:s.recorded,missing:s.missing,startRound:s.startRound});
...
function ensureLeagueLive(){
 const m=state.live;if(!m||m.finished)return null;
 ensureLeagueStatistics();if(!state.leagueStatistics)return null;
 if(!m.leagueBox){
  const friendly=Boolean(m.friendly||state.calendar?.active);
  const game=friendly?{home:managerClub(),away:m.opponent}:state.schedule.find(g=>g.round===state.round&&(g.home===managerClub()||g.away===managerClub()));
  if(!game)return null;
  m.leagueBox={friendly,year:state.season.year,round:state.round,home:game.home,away:game.away,stage:game.seriesId?'playoffs':'regular',players:{},partial:Boolean(m.minute||m.second||m.period>1),saved:false};
 }
 return m.leagueBox;
}
...
function leagueCommitLive(){
 ...
 if(!m?.finished||m.friendly||!box||box.friendly||box.saved||box.year!==state.season.year)return;
...
function leagueStatLeague(){return Object.hasOwn(activeLeagueNames(),leagueStatsUI.league)?leagueStatsUI.league:leagueOf();}
function setLeagueStats(key,value){
 ...
 else if(allowed[key]){leagueStatsUI[key]=value;if(key==='league'){leagueStatsUI.club='all';state.world.selected=value;}if(key==='view'){leagueStatsUI.sort=value==='goalies'?'savePct':value;leagueStatsUI.direction=-1;}}
...
function leagueStatisticsView(){
 ensureLeagueStatistics();const u=leagueStatsUI,s=leagueStatSource(),players=leagueStatPlayers(),goalies=u.view==='goalies',count=s.recorded[u.stage][leagueStatLeague()]||0,missing=s.missing[leagueStatLeague()]||0;
