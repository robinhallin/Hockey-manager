...
const allPlayers=run('Object.values(state.clubRosters).flat().length+(state.playerWorld?.freeAgents||[]).length');
run('ensureRecruitment();save();render()');assert.equal(run('Object.values(state.clubRosters).flat().length+state.loans.external.length+(state.playerWorld?.freeAgents||[]).length'),allPlayers);
...
assert.equal(run('new Set([...Object.values(state.clubRosters).flat(),...state.loans.external,...(state.playerWorld?.freeAgents||[])].map(p=>String(p.id))).size'),allPlayers);
...
assert.equal(migrated.run('Object.values(state.clubRosters).flat().length+state.loans.external.length+(state.playerWorld?.freeAgents||[]).length'),allPlayers);
