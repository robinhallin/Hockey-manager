...
run("globalThis.identityTarget=state.clubRosters[RECRUIT_CLUBS[0][0]][4];playerSocialIdentity(identityTarget);identityTarget.social.ambition=18;identityTarget.social.loyalty=16;identityTarget.social.sensitivity=15;identityTarget.social.trust=82;globalThis.identityWishes=recruitPlayerWishes(identityTarget);globalThis.identityProfile=playerPreferenceProfile(identityTarget)");
...
run('globalThis.rivalTarget=state.clubRosters[RECRUIT_CLUBS[2][0]][5];rivalTarget.transferListed=true;globalThis.w=recruitPlayerWishes(rivalTarget);submitRecruitOffer(rivalTarget.id,recruitFee(rivalTarget),w.salary,2,w.role);globalThis.deal=state.recruitment.deals[0];globalThis.rivalClub=Object.keys(state.recruitment.ai).find(c=>c!==deal.seller);state.recruitment.ai[rivalClub].cash=100000000;state.recruitment.ai[rivalClub].wageLimit=100000000;for(const p of state.clubRosters[rivalClub].filter(p=>worldGroup(p)===worldGroup(rivalTarget))){p.promisedRole="Rotation";p.squadRole="Rotation";}clubAIState(rivalClub).scouting[rivalTarget.id]={visits:3,snapshot:{...rivalTarget.attributes},date:state.calendar.date};deal.rival={club:rivalClub,fee:deal.fee,salary:w.salary*3,years:2,role:"Nyckelspelare"};globalThis.rivalCash=state.recruitment.ai[rivalClub].cash;state.round++;calendarStep(true);calendarStep(true);advanceScoutReports()');
