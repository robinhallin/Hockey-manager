...
function rivalRoleSelection(p){
 const role=p.promisedRole||p.squadRole||'Rotation',r=p.aiRoleReview;
 if(!['Ordinarie','Nyckelspelare'].includes(role)||!r||!medicalReady(p)||(p.fatigue||0)>=35)return 0;
 const target=aiRoleDemand(p,role);
 if(p.pos==='MV'){
  const samples=r.startSamples||0;if(samples<2)return 0;
  return Math.min(.8,Math.max(0,target*samples/6-(r.starts||0))*.4);
 }
 const samples=(r.games||0)%6;
 if(samples<2&&(r.games||0)<6)return 0;
 const average=samples?(r.seconds||0)/samples:r.lastAverage;
 if(!Number.isFinite(average))return 0;
 return Math.min(.6,Math.max(0,1-average/target)*.6);
}
function rivalLineup(club,opponentName=null){
 ...
 const scores=new Map(pool.map(p=>[p,rivalRating(p,p.pos==='B'?'defense':plan.style==='counter'?'defense':'attack')+(p.aiForm||0)*.18+(c?.coach.youth&&p.age<=23?.4:0)+rivalRoleSelection(p)])),score=p=>scores.get(p)||0;
 ...
 const centers=pool.filter(p=>!['MV','B'].includes(p.pos)&&positionFit(p,'C')>=.98).sort((a,b)=>aiUnitScore(b,'C')+rivalRoleSelection(b)-aiUnitScore(a,'C')-rivalRoleSelection(a));
 ...
 const keeperScore=p=>rivalRating(p)+(p.aiForm||0)*.25-(c?.recent.slice(-3).filter(g=>samePlayerId(g.keeper,p.id)).length||0)*.65+rivalRoleSelection(p);
...
function rivalLiveDecision(){
 ...
 const decision=aiCoachDecision(m.opponent,a.basePlan||{style:a.baseStyle,rotation:a.rotation,tempo:'normal'},
  aiObserveMatch(a,{seconds:elapsed,gf:m.opp,ga:m.hv,shots:e?.stats[1].shots||m.shotsOpp||0,againstShots:e?.stats[0].shots||m.shotsHV||0,flow:e?.stats?{own:e.stats[1],against:e.stats[0]}:null,previous:a,linePoints:[0,1,2,3].map(i=>(a.forwards||[]).slice(i*3,i*3+3).reduce((n,id)=>{const r=m.leagueBox?.players[m.opponent+':'+id];return n+(r?.goals||0)+(r?.assists||0);},0)),energy:(a.forwards||[]).map(id=>(state.clubRosters[m.opponent]||[]).find(p=>samePlayerId(p.id,id))).filter(Boolean).reduce((n,p)=>n+matchEnergy(p),0)/Math.max(1,a.forwards.length),strength:StudioHockey.penaltyCount(m.penaltiesHV)-StudioHockey.penaltyCount(m.penaltiesOpp)}));
...
function rivalInitiativeChance(home,away,homePlan,awayPlan,homeCount=5,awayCount=5){
 const control=a=>a.passing*.35+a.puckControl*.30+a.vision*.20+a.decisions*.15;
 const press=(a,b,plan)=>plan.forecheck==='aggressive'?.015+(StudioHockey.pressureWinChance(a,b)-.24)*.5:0;
 return attrClamp(.51+(control(home)-control(away))*.015+press(home,away,homePlan)-press(away,home,awayPlan)+(homeCount-awayCount)*.07,.25,.75);
}
function rivalInitiativeReport(games){
 const known=games.filter(g=>!g.partial&&g.initiative?.version===1),total=known.reduce((n,g)=>n+g.initiative.evenTotal,0);
 if(!total)return '<p>Anfallsinitiativ följs från nya bakgrundsmatcher vid lika styrka.</p>';
...
function rivalSimulate(game,{regulationOnly=false}={}){
 ...
 const tick=()=>{
  ...
  const teamAttributes=ice.map(ps=>Object.fromEntries(keys.map(k=>[k,ps.length?ps.reduce((n,p)=>n+attribute(p,k),0)/ps.length:1])));
  const initiative=rivalInitiativeChance(teamAttributes[0],teamAttributes[1],sides[0].l.plan,sides[1].l.plan,ice[0].length,ice[1].length);
  const side=rand()<initiative?0:1,b=sides[side],other=sides[1-side];
  for(let i=0;i<2;i++){const record=sides[i].initiative;record.total++;if(i===side)record.attacks++;if(ice[0].length===ice[1].length){record.evenTotal++;if(i===side)record.evenAttacks++;}}
...
function rivalPublicIdentity(club){
 const c=rivalsClubState(club),ai=clubAIState(club),strategy=aiStrategyFor(club),def=AI_STRATEGIES[strategy?.type]||AI_STRATEGIES.identity,roster=state.clubRosters[club]||[],avg=roster.length?roster.reduce((n,p)=>n+p.age,0)/roster.length:0;
 return {strategy:def.name,strategyText:def.text,started:strategy?.started,review:strategy?.review,coach:c?.coach?.name||'Okänd',style:c?.coach?RIVAL_STYLES[c.coach.style]:'Okänd',rotation:c?.coach?RIVAL_ROTATIONS[c.coach.rotation]:'Okänd',avgAge:avg,young:roster.filter(p=>p.age<=23).length,changes:c?.changes||0};
}
function rivalIdentityCard(club){
 const d=rivalPublicIdentity(club);return `<section class="rival-panel rival-public-identity"><h2>Klubbens sportsliga riktning</h2><div class="rival-numbers"><div><span>Flerårsplan</span><strong>${trainingSafe(d.strategy)}</strong><small>${trainingSafe(d.strategyText)}</small></div><div><span>Tränaridentitet</span><strong>${trainingSafe(d.style)}</strong><small>${trainingSafe(d.coach)} · ${trainingSafe(d.rotation)}</small></div><div><span>Trupprofil</span><strong>${d.avgAge.toFixed(1)} år</strong><small>${d.young} spelare ≤23 år · ${d.changes} tränarbyten</small></div></div><p>Strategin är sparad över flera säsonger och omprövas tidigast ${d.review?seasonLabel(d.review):'senare'}. Det här visar klubbens observerbara riktning, inte dolda spelarvärden.</p></section>`;
}
function rivalsView(){
