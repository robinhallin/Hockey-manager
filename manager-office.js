...
function managerOfficeView(){
  ...
  <div class="office-grid"><div class="office-main">
    ...
    <section class="office-panel"><header><h2>Matcher</h2>${deskLink('Hela kalendern',{page:'calendar'})}</header><div class="office-fixtures"><div><h3>Kommande</h3>${officeFixtures(fixtures.upcoming.filter(g=>!active||g.date!==c.date||g.opponent!==state.live.opponent),false)}</div><div><h3>Senast spelade</h3>${officeFixtures(fixtures.recent,true)}${fixtures.recent.length?'<small>¹ Ditt lags mål visas först.</small>':''}</div></div></section>
