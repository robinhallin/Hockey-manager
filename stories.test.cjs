...
run("state.stories.active=[];state.stories.started=[];state.stories.lastStart=-10;state.stories.matchCount=4;globalThis.signalPlayer=managerRoster().find(p=>p.age<=32);signalPlayer.contractYears=1;playerIdentity(signalPlayer).ambition=18;globalThis.signalCreated=storiesSystemSignal()");
