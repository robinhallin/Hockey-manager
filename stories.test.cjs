...
run("state.stories.active=[];state.stories.started=[];state.stories.lastStart=-10;state.stories.matchCount=4;globalThis.signalPlayer=managerRoster().find(p=>p.age<=32);signalPlayer.contractYears=1;playerSocialIdentity(signalPlayer).ambition=18;globalThis.signalCreated=storiesSystemSignal()");
assert.equal(run('signalCreated'),true);assert.ok(run("state.stories.active.some(s=>s.type==='contract')"));
